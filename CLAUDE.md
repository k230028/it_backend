...
- 신청서 상태 전이·결재선 변경처럼 동시 실행이 서로를 덮어쓰는 명령은 원장 행을 `findByIdForUpdate`(`PESSIMISTIC_WRITE`)로 잠근 뒤 수행합니다. 잠금 조회에는 `jakarta.persistence.lock.timeout` 힌트를 두고, 잠금 대기 초과 판정은 `LockTimeouts.isLockTimeout`으로만 하며 409 변환은 Repository가 아니라 Service 가드에서 합니다.
- 사용자가 수정하는 예산 원장(정보화사업·경상사업·전산업무비)은 `*ConcurrencyStamper`가 계산한 `concurrencyStamp`를 상세·개정본 응답에 싣고, 저장 요청은 같은 스탬프를 되돌려 보내야 합니다. `*ConcurrencyGuard`는 행 잠금과 결재 상태 확인 뒤, 원장 수정 직전에 스탬프를 대조하며 누락은 400 `*_STAMP_REQUIRED`, 불일치는 현재 원장을 실은 409 `*_SOURCE_CHANGED`, 잠금 초과는 409 `*_CONCURRENT_UPDATE`로 응답합니다. 반입·생성 경로는 면제하고, 스탬프 입력에서 감사 필드와 `LST_YN`은 제외합니다.
...
- 첨부파일 종류(`APG_FL_KD_NM`)를 새로 쓰려면 읽기 또는 쓰기 판정기를 먼저 등록합니다. 알려진 종류 목록은 손으로 관리하지 않고 판정기가 선언한 종류의 합집합이므로, 판정기 없는 종류가 업로드 경로로 조용히 생기지 않습니다. 쓰기 판정기를 추가할 때는 범용 파일 API의 연결 수정(`allowsGenericMutation`)과 삭제(`allowsGenericDeletion`)를 열어 둘지를 기본값에 맡기지 말고 종류마다 명시적으로 판단합니다. 새 종류는 `FileStoragePathPolicy`의 영문 물리 폴더 매핑에도 함께 등록해야 업로드가 성공하며, 한글 폴더로 저장된 레거시 경로는 `resolveStoredDirectory`가 해석하므로 새 코드가 한글 폴더명을 만들지 않습니다.
- 부점코드의 국내·국외 판정은 `BranchCodes`만 사용하고 `startsWith("9")` 같은 비교를 도메인마다 다시 쓰지 않습니다.
- 인스턴스 간 내부 API의 공유 비밀 헤더는 `MessageDigest.isEqual`로 상수 시간 비교합니다.
...
- 전용 테이블 없이 `BGDOCM` 단일 문서로 운영하는 기능(공통 안내 팝업, 스피드다이얼 담당자 정보 등)은 `DOC_TTL_CONE` 고정 식별자와 그 식별자로 범위를 좁힌 조건부 유일 인덱스로 "활성 1건" 불변식을 지킵니다. 최초 생성은 동시 저장의 유일 제약 실패가 호출자 트랜잭션을 오염시키지 않도록 `REQUIRES_NEW`로 분리합니다. **`DOC_MNG_NO` 접두사는 기능마다 새로 정합니다** — 기존 접두사를 재사용하면 접두사만으로 필터링하는 다른 기능의 목록·상세에 그 문서가 섞여 노출됩니다. 문서 유형은 `BgdocDocumentType`(`DOC_DTL_ITM_C`)에 등록하고 관리번호는 `BgdocNumberAllocator.next(접두사)`로만 채번하며, 활성 유일 인덱스는 `(DOC_DTL_ITM_C, DOC_TTL_CONE)` 조합을 기준으로 합니다.
- 예산 원장의 수정·삭제는 `(원본 테이블, 관리번호, 순번)` 단위로 `ApprovalWriteGuard`를 먼저 통과합니다. 수정은 결재중(1)이면 누구도 할 수 없고, 결재완료(2)·수기등록(9)은 시스템관리자만 사후 정정하며, 반려(3)·회수(4)·작성완료(0)·신청서 없음은 재상신을 위해 열어 둡니다. 삭제는 관리자에게도 예외를 두지 않아 신청서 없음·작성완료·반려·회수 상태에서만 허용합니다.
- 프로세스 로컬 Caffeine 캐시는 무효화가 인스턴스 간에 전파되지 않으므로 TTL이 곧 다른 AP가 옛 값을 보는 최대 시간입니다. 새 캐시의 TTL은 "이 데이터가 다른 AP에서 몇 초까지 옛 값이어도 되는가"를 근거로 정하고, 즉시 전파가 필요한 데이터는 캐시 대신 피어 내부 API나 DB 조회를 사용합니다.
