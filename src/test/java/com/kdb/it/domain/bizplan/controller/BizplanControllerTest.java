 ...
 *
 * <p>사업계획 API의 인증/인가 동작과 HTTP 응답 구조·상태코드 매핑을 검증합니다.
 * 서비스 계층은 {@link MockitoBean}으로 대체하며, 인증은 {@link WithMockUser}로 시뮬레이션합니다.
 * 권한/유효성 예외의 HTTP 상태코드 매핑은 {@code GlobalExceptionHandler}가 담당합니다.</p>
 */
...
class BizplanControllerTest {
    ...
    @MockitoBean
    private BizplanService bizplanService;
    @MockitoBean
    ...
    private BizplanDto.ListItem listItem() {
        return new BizplanDto.ListItem(PRJ, "차세대 시스템 구축", "18001", "정보기술부",
                "2026", new BigDecimal("1650000"), "21", LocalDateTime.of(2026, 3, 1, 10, 0));
    }

    /** 상세 응답 (자식 목록은 비어 있음) */
    private BizplanDto.Detail detail() {
        return new BizplanDto.Detail(PRJ, "차세대 시스템 구축", "BG-2026-0001",
                new BigDecimal("1650000"), "01", "<p>보고서</p>", "21",
    ...
    void save_유효요청_200반환() throws Exception {
        BizplanDto.SaveRequest req = new BizplanDto.SaveRequest(
                "<p>보고서</p>", "01", List.of(), List.of(), List.of());

        mockMvc.perform(put(BASE + "/" + PRJ)
    ...
    void save_필수목록누락_400반환() throws Exception {
        ...
        String invalidBody = "{\"redtConeInf\":\"<p>x</p>\",\"itPtlEdrtTc\":\"01\","
                + "\"items\":[],\"contracts\":[]}";
    ...
    void changeStatus_유효요청_200반환() throws Exception {
        ...
        mockMvc.perform(post(BASE + "/" + PRJ + "/status")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isOk());
    ...
    void changeStatus_잘못된전이_400반환() throws Exception {
        doThrow(new IllegalStateException("작성중(21) 상태에서만 완료할 수 있습니다."))
                .when(bizplanService).complete(eq(PRJ), any(BizplanDto.StatusRequest.class), any());
