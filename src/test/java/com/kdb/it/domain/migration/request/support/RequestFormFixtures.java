...
public final class RequestFormFixtures {
     ...
     *
     * <p>행 구성(0-based 시트 행 → 엑셀 행): 5→6 정상 행, 6~8→7~9 괄호 설명이 붙은 소계·합계·계 집계 행, 9→10 통화 칸 좌우가
     * 모두 빈 집계 행, 10→11 비목 칸을 비워 위 행에서 이어받는 정상 행.
     *
    ...
    public static byte[] generalExpenseSummaryRowsXls() {
        try (Workbook wb = new HSSFWorkbook()) {
            ...
            // 비목 칸에 괄호 설명이 붙은 `소계`를 적은 집계 행
            put(s, 6, 0, "소계 (회선사용료)");
            put(s, 6, 2, "-");
            put(s, 6, 3, "-");
            putNumber(s, 6, 5, 1_200_000d);

            // 세부비목 칸에 괄호 설명이 붙은 `합계`를 적고 계약명·통화를 `-`로 채운 집계 행
            put(s, 7, 1, "합계 (전체)");
            put(s, 7, 2, "-");
            put(s, 7, 3, "-");
            putNumber(s, 7, 5, 1_700_000d);

            // 괄호 안에 산식·설명이 붙은 집계 행
            put(s, 8, 2, "계 (월간 합산)");
            put(s, 8, 3, "-");
            putNumber(s, 8, 5, 1_700_000d);

            // 통화 칸 좌우(계약명·월간)가 모두 빈 집계 행
            putNumber(s, 9, 5, 1_700_000d);

            // 비목 칸을 비운 정상 행 — 위의 `전산 제비`·`회선사용료`를 이어받아야 한다
            put(s, 10, 2, "백업 회선 이용료");
            put(s, 10, 3, "KRW");
            putNumber(s, 10, 5, 500_000d);
            put(s, 10, 6, "LGU+");
            return toBytes(wb);
        } catch (IOException e) {
            throw new UncheckedIOException(e);
