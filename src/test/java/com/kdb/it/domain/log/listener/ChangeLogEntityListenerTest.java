 ...
 *
 * <p>JPA 엔티티 생명주기 콜백(@PrePersist, @PreUpdate, @PostUpdate)이 올바른 변경유형(C/U/D)으로 로그를 저장하는지 검증합니다.
 *
 * <p>ApplicationContextHolder 는 정적 메서드를 사용하므로 MockedStatic 으로 mock 처리합니다. AuditLogPersister 는
 * Mockito mock 으로 주입합니다.
 */
@ExtendWith(MockitoExtension.class)
class ChangeLogEntityListenerTest {

    @Mock private AuditLogPersister auditLogPersister;
    ...
    void setUp() {
        ...
        contextHolderMock
                .when(() -> ApplicationContextHolder.getBean(AuditLogPersister.class))
    ...
    /**
     * @LogTarget 이 붙은 엔티티의 @PrePersist 이벤트 발생 시 AuditLogPersister.persist() 가 변경유형 'C' 로 호출되어야 합니다.
     */
    @Test
    ...
    void onPrePersist_LogTarget엔티티_변경유형C로저장() throws Exception {
        ...
        verify(auditLogPersister, times(1)).persist(eq(entity), eq(SampleLogEntity.class), eq("C"));
    }

    /** delYn 필드가 N 인 엔티티의 @PreUpdate 이벤트 발생 시 변경유형 U 로 로그를 저장해야 합니다. */
    @Test
    ...
    void onPreUpdate_delYnN_변경유형U로저장() throws Exception {
        ...
        verify(auditLogPersister, times(1)).persist(eq(entity), eq(SampleLogEntity.class), eq("U"));
    }

    /** delYn 필드가 Y 인 엔티티의 @PreUpdate 이벤트 발생 시 변경유형 D(논리삭제)로 로그를 저장해야 합니다. */
    @Test
    ...
    void onPreUpdate_delYnY_변경유형D로저장() throws Exception {
        ...
        verify(auditLogPersister, times(1)).persist(eq(entity), eq(SampleLogEntity.class), eq("D"));
    }

    // ---- 실패 케이스 ----

    /**
     * @LogTarget 어노테이션이 없는 엔티티의 @PrePersist 이벤트 발생 시 AuditLogPersister 가 호출되지 않아야 합니다.
     */
    @Test
    ...
    void onPrePersist_LogTarget없는엔티티_로그저장안함() throws Exception {
        ...
        // Assert
        verify(auditLogPersister, never()).persist(any(), any(), any());
    }

    /**
     * @LogTarget 없는 엔티티의 @PreUpdate 이벤트 발생 시 AuditLogPersister 가 호출되지 않아야 합니다.
     */
    @Test
    ...
    /**
     * AuditLogPersister 가 예외를 던지더라도 ChangeLogEntityListener 는 예외를 삼켜서 본 업무 트랜잭션에 영향을 주지 않아야 합니다.
     */
    @Test
    ...
    void onPrePersist_Persister예외_예외삼킴() throws Exception {
        // Arrange
        doThrow(new RuntimeException("DB 오류")).when(auditLogPersister).persist(any(), any(), any());
        SampleEntity entity = new SampleEntity();
    ...
    /** 동일 flush 사이클에서 동일 엔티티 인스턴스에 대해 @PreUpdate 가 두 번 호출될 때 두 번째 호출은 중복 방지를 위해 무시되어야 합니다. */
    @Test
    ...
    void onPreUpdate_동일인스턴스중복호출_첫번째만저장() throws Exception {
        ...
        verify(auditLogPersister, times(1)).persist(any(), any(), any());
    }

    /**
     * @PostUpdate 이후 동일 엔티티에 대해 다시 @PreUpdate 가 호출되면 다음 flush 사이클의 새 업데이트로 로그 저장이 가능해야 합니다.
     */
    @Test
    ...
    void onPostUpdate_PostUpdate이후재업데이트_로그저장가능() throws Exception {
        ...
        verify(auditLogPersister, times(2)).persist(any(), any(), any());
    }

    /** ApplicationContextHolder 가 초기화되지 않아 예외가 발생하더라도 본 업무 트랜잭션에 영향을 주지 않아야 합니다. */
    @Test
    ...
    void persistLog_persister조회실패_recorder에schedule기록() {
        ...
        contextHolderMock
                .when(() -> ApplicationContextHolder.getBean(AuditFailureRecorder.class))
                .thenReturn(recorder);
