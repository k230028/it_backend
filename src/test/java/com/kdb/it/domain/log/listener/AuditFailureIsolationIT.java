 ...
 *
 * <p>대상 엔티티는 외래키가 없는 전산관리비 마스터 {@link Bcostm}(테이블 {@code TPRMPP_BCOSTM})이며, 대응 감사 로그는 {@code
 * TPRMPP_BCOSTL}(시퀀스 {@code SQ_TPRMPP_BCOSTL_1})이다. 각 테스트는 {@code ZZIT}로 시작하는 고유 업무 PK({@code
 * BG_NO})를 직접 생성하고 종료 시 두 테이블에서 자신의 행만 삭제한다.
 *
 * <p>검증 시나리오
 *
 * <ul>
 *   <li>원 업무 커밋 + writer 성공 → 업무 행과 감사 행이 모두 존재한다.
 *   <li>원 업무 롤백 → 업무 행과 감사 행이 모두 없다({@code afterCommit} 미발화).
 *   <li>{@code afterCommit} 감사 쓰기 강제 실패 → 업무 행은 남고 감사 행은 없으며 {@code
 *       audit.log.write.failure{stage=afterCommit}} 카운터가 정확히 1 증가한다.
 *   <li>같은 스레드의 다음 정상 저장 성공 → 재진입 ThreadLocal 잔존이 없음을 증명한다.
 * </ul>
 *
 * <p>{@code afterCommit} 콜백은 커밋 스레드에서 동기로 실행되므로 별도 대기(Awaitility) 없이 {@code execute(...)} 반환 직후 결과를
 * 단언한다. 로컬 Oracle이 꺼져 있으면 {@link OracleAvailableCondition}이 컨텍스트 로드 전에 테스트를 깨끗하게 스킵한다.
 */
@Tag("it")
@SpringBootTest(
        properties = {
...
class AuditFailureIsolationIT {
    ...
    @MockitoSpyBean private AuditLogWriter auditLogWriter;
     ...
     *
     * <p>{@code saveAndFlush}로 트랜잭션 내 INSERT와 {@code @PrePersist}를 확정해 커밋 후 {@code afterCommit} 감사
     * 쓰기를 예약한다. 정상 반환 시 커밋되어 콜백이 동기로 실행된다.
     *
     ...
     *
     * <p>{@code @PrePersist}로 감사 동기화가 예약된 상태에서 롤백해도 {@code afterCommit}이 발화하지 않아 감사 INSERT가 실행되지
     * 않음을 검증하기 위한 경로다. 의도된 롤백 신호는 콜백 밖에서 삼킨다.
     *
     ...
     * {@code @PrePersist}·JPA Auditing이 채운다.
     *
     * @param bgNo 고유 업무 PK
     * @return 저장 대상 엔티티
     */
    ...
    private String uniqueBgNo() {
        String bgNo =
                (TEST_PK_PREFIX + UUID.randomUUID().toString().replace("-", "")).substring(0, 15);
