...
class AdminMenuControllerTest {
    ...
    void getAll_관리자인증_200반환() throws Exception {
        ...
        MenuDto.Node node = MenuDto.Node.builder()
                .mnuId("MNU0000001").mnuNm("예산관리").mnuTpC("GRP").mnuDep(1)
    ...
    void update_정상요청_204반환() throws Exception {
        ...
        // when & then
        mockMvc.perform(put("/api/admin/menus/MNU0000001")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
    ...
    void delete_정상요청_204반환() throws Exception {
        // when & then
        mockMvc.perform(delete("/api/admin/menus/MNU0000001"))
                .andExpect(status().isNoContent());
    }
    ...
    void reorder_정상요청_204반환() throws Exception {
        // given
        MenuDto.ReorderRequest req = MenuDto.ReorderRequest.builder()
                .orderedMnuIds(List.of("MNU0000001", "MNU0000002"))
                .build();

        // when & then
        mockMvc.perform(patch("/api/admin/menus/reorder")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isNoContent());
    }
    ...
    void move_정상요청_204반환() throws Exception {
        // given
        MenuDto.MoveRequest req = MenuDto.MoveRequest.builder()
                .newHrkMnuId("MNU_PARENT").build();

        // when & then
        mockMvc.perform(patch("/api/admin/menus/MNU0000001/move")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isNoContent());
    }
    ...
    void move_순환참조_400반환() throws Exception {
        // given
        MenuDto.MoveRequest req = MenuDto.MoveRequest.builder()
                .newHrkMnuId("MNU_CHILD").build();
        doThrow(new ResponseStatusException(HttpStatus.BAD_REQUEST, "순환 참조"))
                .when(adminMenuService).move(eq("MNU0000001"), eq("MNU_CHILD"));
