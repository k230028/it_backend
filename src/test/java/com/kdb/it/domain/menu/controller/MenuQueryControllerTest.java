 ...
 *
 * <p>사용자용 메뉴 트리 조회 API의 HTTP 응답을 검증한다.</p>
 */
...
class MenuQueryControllerTest {
    ...
    void getMenus_일반사용자_200반환() throws Exception {
        // given
        MenuDto.Node node = MenuDto.Node.builder()
                .mnuId("MNU0000001").mnuNm("공지사항").mnuTpC("LNK")
                .srePth("/board/notice").mnuDep(1).children(List.of()).build();
    ...
    void getMenus_관리자_200반환() throws Exception {
        // given
        MenuDto.Node adminMenu = MenuDto.Node.builder()
                .mnuId("MNU_ADMIN").mnuNm("시스템관리").mnuTpC("GRP")
                .mnuDep(1).children(List.of()).build();
        MenuDto.Node userMenu = MenuDto.Node.builder()
                .mnuId("MNU_USER").mnuNm("공지사항").mnuTpC("LNK")
                .mnuDep(1).children(List.of()).build();
    ...
    void getMenus_중첩트리_200반환() throws Exception {
        // given
        MenuDto.Node child = MenuDto.Node.builder()
                .mnuId("CHILD_1").mnuNm("예산목록").mnuTpC("LNK")
                .srePth("/budget/list").mnuDep(2).children(List.of()).build();
        MenuDto.Node parent = MenuDto.Node.builder()
                .mnuId("PARENT_1").mnuNm("예산관리").mnuTpC("GRP")
                .mnuDep(1).children(List.of(child)).build();
    ...
    void getMenus_CustomUserDetails주체_athIds전달() throws Exception {
        ...
        CustomUserDetails userDetails = new CustomUserDetails("10001",
                List.of(CustomUserDetails.ATH_ADMIN), "D001");
        UsernamePasswordAuthenticationToken auth =
                new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());

        MenuDto.Node node = MenuDto.Node.builder()
                .mnuId("MNU_ADMIN").mnuNm("시스템관리").mnuTpC("GRP").mnuDep(1).children(List.of()).build();
        given(menuQueryService.getMenuTree(List.of(CustomUserDetails.ATH_ADMIN))).willReturn(List.of(node));
