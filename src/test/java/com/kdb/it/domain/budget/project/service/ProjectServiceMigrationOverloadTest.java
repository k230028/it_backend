...
class ProjectServiceMigrationOverloadTest {
    ...
    void 생략플래그가_참이면_기간검증을_호출하지_않는다() {
        ...
        Throwable thrown =
                Assertions.catchThrowable(
                        () -> projectService.createProject(new ProjectDto.CreateRequest(), true));
    ...
    @Test
    @DisplayName("1-인자 createProject는 기간 검증을 그대로 수행한다")
    void 기존시그니처는_기간검증을_유지한다() {
        ...
        Assertions.assertThatThrownBy(
                        () -> projectService.createProject(new ProjectDto.CreateRequest()))
