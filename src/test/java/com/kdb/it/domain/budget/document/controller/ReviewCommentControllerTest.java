 ...
 *
 * <p>검토의견 HTTP 응답 구조와 인증 동작을 검증합니다.
 */
...
class ReviewCommentControllerTest {
    ...
    @MockitoBean private ReviewCommentService reviewCommentService;
    ...
    void getComments_비인증_401() throws Exception {
        mockMvc.perform(get("/api/documents/" + DOC_ID + "/review-comments").param("docVrs", "1.0"))
    ...
    void addComment_인증_201() throws Exception {
        given(reviewCommentService.addComment(anyString(), any())).willReturn(null);
        mockMvc.perform(
                        post("/api/documents/" + DOC_ID + "/review-comments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"docVrs\":1.0,\"itPtlRplOpnnTc\":\"G\",\"ivgOpnnCone\":\"테스트 의견\"}"))
                .andExpect(status().isCreated());
    }

    @Test
    @DisplayName(
            "PATCH /api/documents/{docMngNo}/review-comments/{ivgSno}/resolve - 인증된 사용자 → 204 No Content")
    @WithMockUser(username = "10001")
