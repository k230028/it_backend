 ...
 *
 * <p>BbugtmRepository, CodeRepository를 Mock 처리하여 Oracle DB 없이 편성비목 조회, 편성률 적용, 편성 결과 조회 로직을 검증합니다.
 *
 * <p>핵심 검증 대상:
 *
 * <ul>
 *   <li>extractPrefix: "DUP-IOE-237" → "IOE-237" 변환
 *   <li>calculateDupBg: 요청금액 × (편성률/100) 계산 (HALF_UP 반올림)
 *   <li>getIoeCategories: IOE 코드별 편성률 + 요청금액 반환
 *   <li>getSummary: 비목별 요약 + 합계 반환
 * </ul>
 */
...
class BudgetWorkServiceTest {
    ...
    void buildPrefixToIoeCValuesMap_누락값건너뜀() {
        ...
        java.util.Map<String, java.util.Set<String>> result =
                budgetWorkService.buildPrefixToIoeCValuesMap(codes);
