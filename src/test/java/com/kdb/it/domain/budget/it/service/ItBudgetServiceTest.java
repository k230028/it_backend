...
class ItBudgetServiceTest {
    ...
    void getSummary_returnsRows() {
        var row = new ItBudgetDto.CategoryRow(
    ...
    void getComparison_calculatesDiffRate() {
        given(itBudgetQueryRepository.findSummary("2026")).willReturn(List.of(
                new ItBudgetDto.CategoryRow(
                        "001", "351-1100", "개발비(일반)", "개발비", "전산용역비", true, 150, 0, 50, 0, 200, 0)
        ));
        given(itBudgetQueryRepository.findSummary("2025")).willReturn(List.of(
                new ItBudgetDto.CategoryRow(
                        "001", "351-1100", "개발비(일반)", "개발비", "전산용역비", true, 100, 0, 0, 0, 100, 0)
        ));
    ...
    void getComparison_blankAbbrNm_fallsBackToCodeNm() {
        given(itBudgetQueryRepository.findSummary("2026")).willReturn(List.of(
                new ItBudgetDto.CategoryRow(
                        "009", "239-0200", "외주용역(자문/심사)", null, "전산용역비",
                        false, 10, 0, 0, 0, 10, 0)
        ));
        given(itBudgetQueryRepository.findSummary("2025")).willReturn(List.of());
    ...
    void getComparison_prevMissing_hasNullRate() {
        given(itBudgetQueryRepository.findSummary("2026")).willReturn(List.of(
                new ItBudgetDto.CategoryRow(
                        "002", "240-0200", "유지보수료", "유지보수료", "전산제비", false, 30, 0, 0, 0, 30, 0)
        ));
        given(itBudgetQueryRepository.findSummary("2025")).willReturn(List.of());
