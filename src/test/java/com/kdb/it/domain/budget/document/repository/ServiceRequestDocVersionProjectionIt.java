...
class ServiceRequestDocVersionProjectionIt extends AbstractOracleRepositoryTest {
    ...
    void 활성버전만버전내림차순으로조회하고대용량본문은읽지않는다() {
        ...
        List<ServiceRequestDocRepository.VersionHistoryView> views =
                repository.findAllProjectedByDocMngNoAndDelYnOrderByDocVrsSnoDesc(DOC_MNG_NO, "N");

        assertThat(views).hasSize(3);
        assertThat(views).extracting(view -> view.getDocMngNo()).containsOnly(DOC_MNG_NO);
        assertThat(views)
                .extracting(view -> view.getDocVrsSno())
                .containsExactly(
                        new BigDecimal("200"), new BigDecimal("101"), new BigDecimal("100"));
        assertThat(views)
                .extracting(view -> view.getFstEnrDtm())
                .containsExactly(
                        LocalDateTime.of(2026, 7, 21, 11, 0),
                        LocalDateTime.of(2026, 7, 21, 10, 0),
                        LocalDateTime.of(2026, 7, 21, 9, 0));
        assertThat(views)
                .extracting(view -> view.getLstChgDtm())
                .containsExactly(
                        LocalDateTime.of(2026, 7, 21, 11, 5),
                        LocalDateTime.of(2026, 7, 21, 10, 5),
                        LocalDateTime.of(2026, 7, 21, 9, 5));
    ...
    private Brdocm document(
            int storedVersion, String delYn, LocalDateTime createdAt, LocalDateTime updatedAt) {
