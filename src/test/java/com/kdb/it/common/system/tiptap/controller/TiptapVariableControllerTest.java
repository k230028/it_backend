 ...
 *
 * <p>Tiptap 변수 카탈로그 조회/토큰 해석 API의 HTTP 응답 구조와 인증·검증 동작을 검증합니다. Design Ref: §2.2 백엔드, §4.5 권한 필터링
 */
...
class TiptapVariableControllerTest {
    ...
    @MockitoBean private TiptapVariableService service;
    ...
    void getMetadata_anonymous_returnsUnauthorized() throws Exception {
        mockMvc.perform(get("/api/tiptap-variables/metadata")).andExpect(status().isUnauthorized());
    }
    ...
    void resolve_validRequest_returnsResults() throws Exception {
        given(service.resolve(any(), any()))
                .willReturn(
                        new ResolveResponse(
                                Map.of("2026.itBudget.requestAmount", ResolvedValue.ok("900억원"))));
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.results.['2026.itBudget.requestAmount'].value").value("900억원"));
    }
    ...
    void resolve_tooManyTokens_returnsBadRequest() throws Exception {
        List<String> tooMany =
                IntStream.range(0, 201).mapToObj(i -> "2026.itBudget.requestAmount").toList();
