 ...
 *
 * <p>로그인 이력 조회 서비스의 2개 메서드(최대 50건 조회, 최근 10건 조회)가 리포지토리에 정확히 위임하고 DTO로 변환하는지 검증합니다. Clognh 엔티티는
...
class LoginHistoryServiceTest {
    ...
    @InjectMocks private LoginHistoryService loginHistoryService;
    ...
    void getLoginHistory_이력있음_DTO목록반환() {
        ...
        given(loginHistoryRepository.findTop50ByEnoOrderByLgnDtmDesc(eno))
    ...
    void getLoginHistory_이력없음_빈목록반환() {
        // given
        given(loginHistoryRepository.findTop50ByEnoOrderByLgnDtmDesc("E99999"))
                .willReturn(List.of());
    ...
    void getRecentLoginHistory_이력없음_빈목록반환() {
        // given
        given(loginHistoryRepository.findTop10ByEnoOrderByLgnDtmDesc("E99999"))
                .willReturn(List.of());
