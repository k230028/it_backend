 ...
 *
 * <p>JWT 클레임에서 복원되는 사용자 권한 객체의 기본 자격등급, 권한 매핑, UserDetails 계약을 검증합니다.
 */
class CustomUserDetailsTest {
    ...
    void authorities_부서관리자_역할매핑() {
        CustomUserDetails details =
                new CustomUserDetails("10001", List.of(CustomUserDetails.ATH_DEPT_MGR), "D001");
    ...
    void infoSecAdmin_협의회관리권한_전용역할매핑() {
        CustomUserDetails details =
                new CustomUserDetails(
                        "10002", List.of(CustomUserDetails.ATH_INFOSEC_ADMIN), "D002");
    ...
    void normalAndUnknownAuthorities_관리권한없음_사용자역할매핑() {
        CustomUserDetails normal =
                new CustomUserDetails("10003", List.of(CustomUserDetails.ATH_USER), "D003");
        CustomUserDetails unknown = new CustomUserDetails("10004", List.of("UNKNOWN"), "D004");
    ...
    private List<String> authorityNames(CustomUserDetails details) {
        return details.getAuthorities().stream().map(value -> value.getAuthority()).toList();
    }
}
