 ...
 * 실시간 로그 컨트롤러 권한 테스트.
 *
 * <p>WebMvcTest 슬라이스에서 ROLE_ADMIN/USER/Anonymous별 접근 제어를 검증한다.
 */
...
class RealtimeLogControllerTest {
    ...
    @MockitoBean private RealtimeLogService service;
    ...
    void anonymous_unauthorized() throws Exception {
        mvc.perform(get("/api/admin/realtime-logs")).andExpect(status().isUnauthorized());
    }
}
