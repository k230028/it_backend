 ...
 *
 * <p>실제 보안 체인({@link SecurityConfig} + {@link JwtAuthenticationFilter})을 그대로 적용해, 백엔드 권한 판단이
 * httpOnly {@code accessToken} JWT에만 기반함을 증명합니다. 프론트 가드용 {@code it-portal-user} 쿠키는 백엔드 인가에 사용되지
 * 않으므로, 위조된 {@code it-portal-user} 쿠키만으로(유효 JWT 없이) {@code /api/admin/**}에 접근하면 {@link
 * JwtAuthenticationFilter}가 인증을 설정하지 않아 401을 반환해야 합니다.
 *
 * <p>{@code TestSecurityConfig}(우회용) 대신 실제 {@code SecurityConfig}와 {@code JwtAuthenticationFilter}를
