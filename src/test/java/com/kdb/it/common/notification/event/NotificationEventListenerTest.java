 ...
 *
 * <p>명시적 알림과 결재 결과·회수 이벤트의 알림 변환 및 실패 격리를 검증합니다.
 */
...
class NotificationEventListenerTest {
    ...
    void onNotificationEvent_정상요청_서비스전달() {
        NotificationEvent event =
                NotificationEvent.builder().recipientEno("10001").itPtlInfmSvcTc("01").build();
    ...
    void onApprovalCompleted_완료_알림변환() {
        ...
        assertThat(captor.getValue().itPtlInfmSvcTc())
                .isEqualTo(NotificationEvent.TYPE_APPROVAL_RESULT);
    ...
    void onApprovalCompleted_조회실패_예외흡수() {
        given(applicationRepository.findById("APF-1"))
    ...
    void onApprovalRecalled_다중대상_유효수신자발송() {
        ...
        given(applicationRepository.findById("APF-1")).willReturn(Optional.of(application));
        given(outboxService.enqueue(any())).willReturn("INF-1");
        ApprovalRecalledEvent event =
                new ApprovalRecalledEvent(
        ...
        assertThat(captor.getAllValues())
                .extracting(value -> value.recipientEno())
                .containsExactly("REQUESTER", "APPROVER");
        assertThat(captor.getAllValues())
                .allSatisfy(
                        notification ->
                                assertThat(notification.itPtlInfmSvcTc())
                                        .isEqualTo(NotificationEvent.TYPE_APPROVAL_RECALLED));
    }
    ...
    void onApprovalRecalled_본인회수_발송하지않음() {
        Capplm application =
                Capplm.builder().apfMngNo("APF-1").dcdReqTtl(null).dcdReqUsid("10001").build();
        given(applicationRepository.findById("APF-1")).willReturn(Optional.of(application));
    ...
    void onApprovalRecalled_발송실패_예외흡수() {
        Capplm application =
                Capplm.builder().apfMngNo("APF-1").dcdReqTtl("신청서").dcdReqUsid("10001").build();
        given(applicationRepository.findById("APF-1")).willReturn(Optional.of(application));
