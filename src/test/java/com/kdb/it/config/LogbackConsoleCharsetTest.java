 ...
 * 콘솔 로그 인코딩 회귀 테스트.
 *
 * <p>{@code logback-spring.xml}의 콘솔 appender는 charset을 {@code ${stdout.encoding:-UTF-8}}로 지정해, JVM이
 * 감지한 콘솔 인코딩(JDK 18+: Windows=MS949, Linux=UTF-8)에 자동 정합된다. 고정 UTF-8이면 Windows 한글 콘솔에서 한글이 깨진다(예:
 * "인증서버" → "?몄쬆?쒕쾭").
 *
 * <p>본 테스트는 동일한 charset 표현식을 plain logback 설정(springProfile 미사용)으로 로드해, logback이 {@code
 * stdout.encoding} 시스템 속성을 실제 {@link Charset}으로 치환하는지(이 수정의 핵심) 검증한다. {@code springProfile} 태그는
 * Spring 확장이 필요해 단독 Joran 로드가 불가하므로 제외한다.
 */
class LogbackConsoleCharsetTest {
    ...
    void consoleCharset_resolvesFromStdoutEncoding() throws Exception {
        ...
        ConsoleAppender<?> console =
                (ConsoleAppender<?>) context.getLogger("ROOT").getAppender("CONSOLE");
        ...
        Charset expected =
                (stdoutEncoding != null && Charset.isSupported(stdoutEncoding))
