 ...
 *
 * <p>실제 {@link SecurityConfig}·{@link JwtAuthenticationFilter}·{@link JwtUtil}과 Actuator 자동설정을
 * 함께 띄워 운영과 동일한 인가 경계를 검증합니다. Oracle·JPA 자동설정은 {@code test} 프로파일
 * ({@code application-test.properties})에서 제외되므로 DB 없이 실행되며 {@code ./gradlew test}
 * 게이트에 포함됩니다.</p>
 *
 * <p>브라우저 인증 경로와 동일하게 {@code accessToken} httpOnly 쿠키에 담긴 유효 JWT로만 권한을
 * 판단함을 확인합니다.</p>
 *
 * <ul>
 * <li>비인증 {@code /actuator/health} → 200 (공개)</li>
 * <li>비인증 {@code /actuator/metrics} → 401 (인증 필요)</li>
 * <li>일반 사용자 {@code /actuator/metrics} → 403 (관리자 전용)</li>
 * <li>관리자 {@code /actuator/metrics/audit.log.write.failure} → 200 (카운터 생성 후)</li>
 * </ul>
 */
...
class SecurityConfigTest {
    ...
    void metrics_admin_returnsCounter200() throws Exception {
        ...
        auditFailureRecorder.record("Bprojm", "PRJ-1", "U", "afterCommit",
                new RuntimeException("테스트 감사 실패"));

        mockMvc.perform(get("/actuator/metrics/" + AUDIT_FAILURE_METRIC)
     ...
     *
     * <p>{@code test} 프로파일이 DataSource/JPA 자동설정을 제외하므로 DB 없이 컨텍스트가 로드됩니다.
     * 실제 {@link SecurityConfig}·JWT 컴포넌트·{@link AuditFailureRecorder}만 등록하고,
     * {@code MeterRegistry}와 Actuator 엔드포인트는 자동설정으로 확보합니다.</p>
     */
