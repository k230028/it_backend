 ...
 *
 * <p>신규 {@link EaiMessageBuilder}와 바이트 단위로 동일함을 증명하기 위한 오라클이다. 신규 빌더의 헬퍼를 재사용하지 않고 자체 구현을 가진다(독립성).
 * 이 파일은 가급적 수정하지 않는다.
 */
class EpamsReferenceMessageBuilder {
    ...
    byte[] buildUms(UmsPayload u, String ifId) {
        ...
        String whl =
                pad("N", 8, String.valueOf(len(p01 + p02 + p03 + p04 + p05 + p06 + p07 + p08)));
    ...
    private String p07(UmsPayload u) {
        ...
        return pad("C", 7, umsBzDttId)
                ...
                + pad("C", 100, reqUsrNm)
                + pad("C", 30, "")
                + pad("C", 200, umsSdChnNo)
                + pad("C", 4000, "")
                + pad("C", 8, (sendDt == null) ? trDt : sendDt)
                + pad("C", 6, (sendTime == null) ? "" : sendTime)
                + pad("C", 100, umsTmeChnNo)
                + props.appC()
                + props.appBzLv1C()
                + pad("C", 14, reqUsid)
                + pad("C", 100, reqUsrNm)
