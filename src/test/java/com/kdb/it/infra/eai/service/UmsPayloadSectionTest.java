 ...
 *
 * <ul>
 *   <li>{@code supports()} 분기: UmsPayload → true, GwePayload → false
 *   <li>{@code systemCode()} 반환값
 *   <li>{@code build()} 내부 분기: umsBzDttId 첫 글자 'E'(이메일) → umsTmeChnNo 및 umsSdChnTpC 분기
 *   <li>{@code variableDataJson()} 분기: umData null/빈 스킵, 값 있음, 특수문자 이스케이프
 *   <li>sendDt null/non-null 분기, sendTime null/non-null 분기
 * </ul>
 */
...
class UmsPayloadSectionTest {
    ...
    void setUp() {
        ...
        UnaryOperator<String> dateFn =
                pattern -> DateTimeFormatter.ofPattern(pattern, Locale.ROOT).format(fixed);
