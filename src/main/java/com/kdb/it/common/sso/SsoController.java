 ...
 *   <li>{@code /sso/checkauth}: ESSO 콜백. 토큰 검증 후 사번을 세션에 저장하고 {@code loginProc}로 이동.
 *       CS 모드는 {@code saveToken.html} 저장 후 서버 등록 완료 URL({@code agentProc})로 복귀.</li>
 *   <li>{@code /sso/loginProc} / {@code /sso/agentProc}: SSO 세션 결과 검증 후 JWT 발급 단계로 연결
 *       ({@code agentProc}는 레퍼런스 SA-WEB 완료 페이지명과 정합되는 별칭).</li>
...
public class SsoController {
     ...
     *
     * <p>{@code SecurityConfig}가 이 값의 sha256을 계산해 CSP {@code script-src}에 등록하므로,
     * 이 스크립트만 인라인 실행이 허용됩니다. 본 상수를 변경하면 CSP 해시도 자동으로 따라갑니다.</p>
     */
    ...
    public void business(@RequestParam(value = "next", required = false) String next,
        ...
        if (next != null && !next.isBlank()) {
            response.addHeader(HttpHeaders.SET_COOKIE, cookieUtil.createSsoNextCookie(next).toString());
        }
        if (origin != null && !origin.isBlank()) {
            response.addHeader(HttpHeaders.SET_COOKIE, cookieUtil.createSsoOriginCookie(origin).toString());
        }
     ...
     * 사번을 세션에 저장하고 {@code loginProc}로 이동합니다. 검증 실패·비정상 호출은 프론트 수동
     * 로그인({@code /login?error=sso})으로 폴백합니다(SSO 재시작 시 checkauth↔business 무한 루프를
     * 피하기 위해 진입점으로 되돌리지 않습니다). ESSO가 GET/POST 중 무엇으로 콜백할지 환경에 따라
     * 다르므로 둘 다 허용합니다.</p>
     *
    ...
    public void checkauth(@RequestParam(value = "resultCode", required = false) String resultCode,
        ...
        if (SSO_SUCCESS_CODE.equals(result.resultCode())) {
            ...
            if (result.useCSMode()) {
                ...
                log.info("SSO checkauth 성공(CS 모드) - 토큰 저장 페이지로 POST: {}", ssoProperties.saveTokenUrl());
                writeAutoSubmitPostForm(response, ssoProperties.saveTokenUrl(),
                        result.resultCode(), secureSessionId);
            } else {
                log.info("SSO checkauth 성공 - loginProc로 이동 (resultData 보유: {})", !result.resultData().isBlank());
                response.sendRedirect("/sso/loginProc");
            }
            return;
        }
        ...
        log.warn("SSO 토큰 검증 실패 - resultCode: {}, IP출처[remoteAddr={}, X-Forwarded-For={}, X-Real-IP={}, Proxy-Client-IP={}, WL-Proxy-Client-IP={}]",
                result.resultCode(),
     ...
     *
     * <p>레퍼런스 Web Agent(SA-WEB)의 완료 페이지가 {@code agentProc.jsp}이고, ISign+ 서버는
     * agent 등록값(returnUrl)으로 이 경로를 호출합니다. CS 모드는 {@code saveToken.html} 저장 후
     * 등록된 완료 URL({@code .../sso/agentProc})로 복귀하므로, 서버 등록 경로 규약과 정합되도록
     * {@code /sso/loginProc}와 동일하게 처리합니다. ESSO가 GET/POST 중 무엇으로 호출할지 환경에
     * 따라 다르므로 둘 다 허용합니다.</p>
     *
     * <p>주의: 완료 URL의 <b>호스트</b>(예: {@code uac.kdb.co.kr:20443})는 ISign+ 서버의 agent
     * 등록값으로, 애플리케이션이 보내는 값이 아닙니다. 실제 배포 호스트와 다르면 SSO 관리자가
     * agent 등록 URL을 수정해야 합니다.</p>
     *
     * @param request  SSO 세션 결과를 읽기 위한 요청
     * @param response JWT 발급 완료 엔드포인트로 이동시키는 Servlet 응답
     * @throws IOException 리다이렉트 응답 작성 실패 시
     */
    @RequestMapping(value = "/sso/agentProc", method = {RequestMethod.GET, RequestMethod.POST})
    public void agentProc(HttpServletRequest request,
                          HttpServletResponse response) throws IOException {
        proceedToComplete(request, response, "agentProc");
    }

    /**
     * 인증 결과 세션을 검증해 {@code ssoVerifiedEno}로 승격하고 {@code complete}로 이동합니다.
     * {@code loginProc}/{@code agentProc} 공통 처리입니다.
     *
     * @param request  SSO 세션 결과를 읽기 위한 요청
     * @param response 리다이렉트 응답
     ...
     * @throws IOException 리다이렉트 응답 작성 실패 시
     */
    private void proceedToComplete(HttpServletRequest request,
    ...
    public void complete(@RequestParam(value = "eno", required = false) String eno,
        ...
        String effectiveOrigin = firstNonBlank(origin, readCookie(request, CookieUtil.SSO_ORIGIN_COOKIE));
        try {
     ...
     * POST(폼 전송)로 받아야 합니다(레퍼런스 {@code checkauth.jsp}의 auto-submit form과 동일).
     * GET 리다이렉트로는 페이로드가 전달되지 않아 CS 모드 토큰 저장이 실패합니다.</p>
     *
     * <p>인라인 제출 스크립트는 {@code SecurityConfig}의 CSP {@code script-src} sha256 해시로만
     * 허용되므로 본문은 정확히 {@code document.forms[0].submit()}이어야 합니다(변경 시 해시도 갱신).
     * 사용자 입력값({@code secureSessionId})은 속성 컨텍스트로 이스케이프해 XSS를 방지합니다.</p>
     *
     * @param response        응답
     ...
     */
    private void writeAutoSubmitPostForm(HttpServletResponse response, String actionUrl,
                                         String resultCode, String secureSessionId) throws IOException {
        response.setContentType("text/html;charset=UTF-8");
        String html = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"></head><body>"
                + "<form name=\"sendForm\" method=\"post\" action=\"" + attr(actionUrl) + "\">"
                + "<input type=\"hidden\" name=\"agentId\" value=\"" + attr(ssoProperties.agentId()) + "\"/>"
                + "<input type=\"hidden\" name=\"errCode\" value=\"" + attr(resultCode) + "\"/>"
                + "<input type=\"hidden\" name=\"secureSessionId\" value=\""
                + attr(secureSessionId == null ? "" : secureSessionId) + "\"/>"
                + "</form>"
                + "<script>" + CS_MODE_SUBMIT_SCRIPT + "</script>"
                + "</body></html>";
