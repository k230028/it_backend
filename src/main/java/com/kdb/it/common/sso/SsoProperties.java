 ...
 *
 * <p>JSP Web Agent(별첨1 SA-WEB)의 {@code agentInfo.jsp}/{@code config.properties} 값을 Spring 설정으로 옮긴
 * 것입니다. {@code sso.*} 프리픽스로 바인딩됩니다.
 *
 * <p>망 분리: PC(브라우저) → ESSO 통신용 URL({@code browserBaseUrl})과 업무서버(AP) → ESSO 통신용 URL({@code
 * hostBaseUrl})이 다를 수 있어 분리합니다(인터넷망/스마트워크망 구성 대응).
 *
 * @param mockEnabled 모의 모드 여부. true이면 ESSO 통신 없이 {@code mockEno}로 로그인합니다 (SSO와 통신 불가한 외부망 개발용). 운영
 *     프로파일은 반드시 false.
