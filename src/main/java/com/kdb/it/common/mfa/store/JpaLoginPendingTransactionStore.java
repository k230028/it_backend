 ...
 *
 * <p>상태 컬럼이 없어 1회 소비를 조건부 물리 삭제로 구현한다. 삭제 전 조회는 반환값 구성용이며, 실제 "정확히 한 번" 보장은 {@link
 * LoginPendingTransactionJpaRepository#consumeOnce}의 영향 행 수가 담당한다 (동시 요청이 같은 행을 봐도 DELETE는 한 트랜잭션만
 * 성공한다).
 */
@Component
...
public class JpaLoginPendingTransactionStore implements LoginPendingTransactionStore {
    ...
    public Optional<LoginPendingTransaction> consumeOnce(
        ...
        Optional<LoginPendingTransactionEntity> found =
                repository.findActiveByTokenHash(tokenHash, nowDtm);
