 ...
 *
 * <p>{@link com.kdb.it.common.mfa.domain.LoginPendingTransaction}의 영속 표현이다. 상태 컬럼이 없어 1회 소비는 {@link
 * LoginPendingTransactionJpaRepository}의 조건부 물리 삭제로 표현한다.
 */
@Entity
...
public class LoginPendingTransactionEntity extends BaseEntity {
    ...
    /**
     * 신규 로그인 대기 거래를 생성한다. 로그인 1단계는 비인증 흐름이므로 소유자 사번을 감사자로 명시적으로 기록한다.
     *
     ...
     */
    public static LoginPendingTransactionEntity create(
            String tokenHash, String eno, LocalDateTime endDtm) {
        LoginPendingTransactionEntity entity =
                LoginPendingTransactionEntity.builder()
                        .tokenHash(tokenHash)
                        .eno(eno)
                        .endDtm(endDtm)
                        .build();
