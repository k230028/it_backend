 ...
 *
 * <p>정확성은 이 배치와 무관하다. 모든 조회·UPDATE 조건에 {@code END_DTM > now}가 들어가므로 만료 행은 어떤 경로로도 사용되지 않는다. 유예 10분은
 * 만료 직후 요청이 사유(만료/미존재)를 정확히 구분할 수 있게 하는 창이며, 이 정리는 순수한 용량 관리다. 다중 인스턴스에서 여러 인스턴스가 동시에 실행해도 DELETE는
 * 멱등이라 잠금이 필요 없다.
 */
