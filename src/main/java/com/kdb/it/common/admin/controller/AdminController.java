 ...
 * 시스템 관리자 전용 API 컨트롤러
 *
 * <p>모든 엔드포인트는 {@code ROLE_ADMIN} (ITPAD001) 역할 보유자만 접근 가능합니다. SecurityConfig에서 {@code
 * /api/admin/**} 경로에 대해 {@code hasRole("ADMIN")} 설정이 이미 적용되어 있으며, 메서드 레벨에서도 이중으로 보호합니다.
 *
 * <p>클래스 수준 관리자 권한을 적용하고 관리 기능별 서비스 호출을 제공합니다.
 */
@RestController
...
public class AdminController {
    ...
    // =========================================================================
    // 공통코드 관리 (TPRMPP_CCODEM)
    // =========================================================================

    /**
     * 공통코드 목록 조회 삭제되지 않은(DEL_YN='N') 전체 공통코드를 코드순서 오름차순으로 반환합니다.
     *
    ...
    /**
     * 공통코드 삭제 (Soft Delete) DEL_YN='Y' 처리 — 물리 삭제 아님.
     *
     * @param cId 코드ID
     * @param cdva 코드값
     * @param sttDt 시작일자
    ...
    /**
     * 공통코드 일괄 업로드 (Upsert) 코드ID가 이미 존재하면 수정, 없으면 신규 생성합니다.
     *
    ...
    // =========================================================================
    // 자격등급 관리 (TPRMPP_CAUTHI) — M3
    // =========================================================================

    /**
     * 자격등급 목록 조회 삭제되지 않은(DEL_YN='N') 전체 자격등급을 반환합니다.
     *
    ...
    /**
     * 자격등급 추가 ATH_ID 중복 시 400 Bad Request를 반환합니다.
     *
    ...
    /**
     * 자격등급 논리 삭제 (Soft Delete) DEL_YN='Y' 처리 -- 물리 삭제 아님.
     *
     * @param athId 자격등급ID
     * @return 204 No Content
     */
    ...
    // =========================================================================
    // 역할 관리 (TPRMPP_CROLEI) — M4
    // =========================================================================

    /**
     * 역할(사용자-자격등급 매핑) 목록 조회 삭제되지 않은(DEL_YN='N') 전체 역할 목록을 반환합니다.
     *
     * @return 역할 응답 DTO 목록
     */
    @GetMapping("/roles")
    ...
    /**
     * 역할 추가 (사용자에게 자격등급 부여) 복합키(athId+eno) 중복 시 400 Bad Request를 반환합니다.
     *
    ...
    public ResponseEntity<Void> deleteRole(
            @PathVariable("athId") String athId, @PathVariable("eno") String eno) {
        adminService.deleteRole(athId, eno);
        return ResponseEntity.noContent().build();
    }

    // =========================================================================
    // 사용자 관리 (TPRMPP_CUSERI) — M5
    // =========================================================================

    /**
     * 사용자 목록 조회 삭제되지 않은(DEL_YN='N') 전체 사용자를 반환합니다.
     *
    ...
    /**
     * 사용자 추가 ENO 중복 시 400 Bad Request를 반환합니다.
     *
    ...
    /**
     * 사용자 기본정보 수정 password 필드가 포함된 경우 비밀번호도 함께 변경됩니다.
     *
     * @param eno 사원번호
    ...
    public ResponseEntity<Void> updateUser(
            @PathVariable("eno") String eno, @Valid @RequestBody AdminDto.UserRequest req) {
    ...
    // =========================================================================
    // 조직 관리 (TPRMPP_CORGNI) — M6
    // =========================================================================

    /**
     * 조직 목록 조회 삭제되지 않은(DEL_YN='N') 전체 조직을 반환합니다.
     *
     * @return 조직 응답 DTO 목록
     */
    ...
    /**
     * 조직 추가 조직코드 중복 시 400 Bad Request를 반환합니다.
     *
    ...
    public ResponseEntity<Void> updateOrganization(
            @PathVariable("orgC") String orgC, @Valid @RequestBody AdminDto.OrgRequest req) {
    ...
    // =========================================================================
    // JWT 토큰 조회 (TPRMPP_CRTOKM) — M7
    // =========================================================================

    /**
     * JWT 갱신토큰 목록 조회 보안상 토큰값은 앞 20자만 마스킹하여 표시합니다.
     *
    ...
    // =========================================================================
    // 첨부파일 조회 (TPRMPP_CFILEM) — M7
    // =========================================================================

    /**
     * 첨부파일 목록 조회 삭제되지 않은(DEL_YN='N') 전체 첨부파일을 반환합니다.
     *
    ...
    public ResponseEntity<AdminLogDto.LogDetailResponse> getLogDetail(
            @PathVariable("logKey") String logKey, @PathVariable("logSno") String logSno) {
    ...
    /**
     * 일별 로그인 통계 조회 (대시보드용) 최근 30일간 일별 로그인 성공 건수를 집계하여 반환합니다.
     *
