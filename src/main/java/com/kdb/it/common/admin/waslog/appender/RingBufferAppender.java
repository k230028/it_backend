 ...
 *
 * <p>{@code logback-spring.xml}에서 {@code <capacity>}로 버퍼 크기를 주입하며, 그 값은
 * {@code app.was-log.buffer-capacity} 프로퍼티에서 온다. 메시지·스택트레이스는 각각 4000자·8000자에서 절단해
...
public class RingBufferAppender extends AppenderBase<ILoggingEvent> {
     ...
     * 로그 이벤트를 버퍼에 담는다.
     *
     * <p>담기 전에 {@link WasLogMasker}로 토큰·주민등록번호를 가린다 — 이 버퍼는 관리자 화면과 다운로드로 그대로 나가고, 다운로드
     * 파일이 개인 PC로 나가면 추적이 끊긴다. <b>파일 appender의 원문은 가리지 않으므로</b> 서버에 남는 로그로 장애를 조사하는 경로는
     * 그대로다(BE-55).
     *
