 ...
 *
 * <p><b>왜 여기인가</b>: {@code /admin/was-logs}는 링버퍼 원문을 그대로 화면과 다운로드로 내보냅니다. 다운로드 파일이 개인 PC로 나가면 ADMIN
 * 권한과 감사 로그 외에 통제 수단이 없습니다. 적재 경로가 {@link RingBufferAppender#append} 한 곳뿐이라 여기서 가리면 화면·다운로드가 함께
 * 덮입니다(BE-55).
 *
 * <p><b>파일 로그는 가리지 않습니다.</b> 이 마스킹은 링버퍼에 담기는 사본에만 적용되고, 파일 appender가 쓰는 원문은 그대로입니다. 서버에 남는 로그로 장애를
 * 조사하는 경로를 막지 않으려는 의도적인 범위입니다.
 *
 * <p><b>대상을 좁게 잡았습니다</b> — 토큰류와 주민등록번호 둘뿐입니다. 과하게 가리면 장애 조사가 불가능해집니다. 특히 <b>사번은 가리지 않습니다</b>: 어느
 * 사용자의 요청에서 난 오류인지가 추적의 출발점이기 때문입니다.
 */
final class WasLogMasker {
     ...
     */
    private static final Pattern RRN = Pattern.compile("\\b[0-9]{6}[-\\s]?[1-8][0-9]{6}\\b");
     ...
     * 로그 한 줄에서 민감값을 가립니다.
     *
     * <p>대부분의 줄에는 가릴 것이 없으므로, 정규식을 돌리기 전에 값싼 사전 검사로 걸러 냅니다 — 이 메서드는 <b>모든 로그 이벤트</b>가 지나는 자리라 평시
     * 비용이 그대로 처리량이 됩니다.
     *
    ...
    static String mask(String value) {
        ...
        if (containsIgnoreCase(masked, "bearer") || containsIgnoreCase(masked, "basic")) {
            masked =
                    BEARER.matcher(masked)
                            .replaceAll(matcher -> matcher.group(1) + " " + TOKEN_MARK);
        }
    ...
    /** 토큰 마스킹 표시 상수. 테스트가 기대값 비교에 쓴다. */
    static String tokenMark() {
        return TOKEN_MARK;
    }

    /** 주민등록번호 마스킹 표시 상수. 테스트가 기대값 비교에 쓴다. */
