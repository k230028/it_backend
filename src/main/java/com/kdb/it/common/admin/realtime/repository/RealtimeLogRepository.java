 ...
 *
 * <p>QueryDSL 메타모델을 사용하지 않고 EntityManager 네이티브 쿼리로 처리한다.
 */
@Repository
...
public class RealtimeLogRepository {
     ...
     *
     * <p>{@code since}가 null이면 초기 스냅샷, 비-null이면 복합 커서 기반 증분 조회.
     */
    public List<RealtimeLogDto.FeedRow> findFeed(RealtimeLogDto.QueryCondition cond) {
        ...
        for (Object[] r : rows) {
            out.add(
                    new RealtimeLogDto.FeedRow(
                            toStr(r[0]),
                            toStr(r[1]),
                            ((Number) r[2]).longValue(),
                            toStr(r[3]), // CHG_DTT_YN — VARCHAR2(1), Oracle JDBC가 Character 반환 가능
                            toLdt(r[4]), // CHG_DTM — TIMESTAMP, Hibernate 6/Spring Boot 4가
                            // LocalDateTime로 매핑하기도 함
                            toStr(r[5]),
                            toStr(r[6]),
    ...
    /**
     * Oracle JDBC가 VARCHAR2(1) 컬럼을 {@code Character}로 반환하는 경우가 있어 직접 {@code (String)} 캐스트 시
     * ClassCastException이 발생한다. 모든 텍스트 컬럼을 안전하게 String으로 변환한다.
     */
    ...
    /**
     * Oracle TIMESTAMP 컬럼은 환경에 따라 {@link Timestamp} 또는 {@link LocalDateTime}로 반환된다. 어느 쪽이든 안전하게
