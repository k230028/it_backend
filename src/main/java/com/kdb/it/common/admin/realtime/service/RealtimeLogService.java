 ...
 * 실시간 로그 모니터링 화면 서비스.
 *
 * <p>{@link AdminLogService#getTables()}의 허용 LOG_KEY 집합과 변경구분 {@code C/U/D}만 허용한다. 검증 실패 시 {@link
...
public class RealtimeLogService {
    ...
    public RealtimeLogService(
            RealtimeLogRepository repository, AdminLogService adminLogService, Clock clock) {
    ...
    public RealtimeLogDto.Snapshot snapshot(
        ...
        var cond =
                new RealtimeLogDto.QueryCondition(
                        since,
                        cursorLogTbl,
                        cursorLogSno,
                        safeLimit,
        ...
        var tableCounts =
                repository.countByTableSince(serverTime.minusMinutes(TABLE_COUNT_WINDOW_MIN));
        var perMinute =
                repository.perMinuteSince(
                        serverTime.minusMinutes(PER_MINUTE_WINDOW_MIN), serverTime);
