...
/** Tiptap 변수 API DTO 모음. 설계 참조: §3.6 API 스키마 */
public final class TiptapVariableDto {
    ...
    public record ResolvedValue(
            ...
            @Schema(
                            description = "상태",
                            allowableValues = {"OK", "MISSING", "FORBIDDEN", "INVALID"})
                    String status) {
        public static ResolvedValue ok(String value) {
            return new ResolvedValue(value, "OK");
        }

        public static ResolvedValue missing() {
            return new ResolvedValue("", "MISSING");
        }
        ...
        public static ResolvedValue invalid() {
            return new ResolvedValue("", "INVALID");
        }
    }
}
