 ...
 *
 * <p>JSON Web Token(JWT) 기반의 Access Token과 Refresh Token을 생성하고 검증하는 유틸리티입니다.
 *
 * <p>JWT 구조: {@code Header}.{@code Payload}.{@code Signature}
 *
 * <ul>
 *   <li>Header: 알고리즘 정보 ({@code HS256} 등 HMAC-SHA 계열)
 *   <li>Payload(Claims): subject(사번), issuedAt(발급 시각), expiration(만료 시각)
 *   <li>Signature: secretKey로 서명한 값 (위변조 방지)
 * </ul>
 *
 * <p>설정값 ({@code application.properties}):
 *
 * <ul>
...
public class JwtUtil {
     ...
     *
     * <p>{@code @Value}로 {@code application.properties}에서 설정값을 읽어옵니다.
     *
     ...
     *
     * <p>사번을 subject로 하며, 자격등급 목록(athIds)과 부서코드(bbrC)를 클레임에 추가합니다. 한 사용자가 여러 자격등급을 가질 수 있으므로
     * athIds는 JSON 배열로 직렬화됩니다.
     *
     * <p>Claims 구성:
     *
     * <ul>
     *   <li>{@code sub}: 사번 (subject)
     *   <li>{@code athIds}: 자격등급 ID 목록 (JSON 배열, 예: ["ITPZZ001", "ITPZZ002"])
     *   <li>{@code bbrC}: 소속 부서코드 (권한 범위 결정용)
     *   <li>{@code iat}: 발급 시각 (issued at)
     *   <li>{@code exp}: 만료 시각 (expiration)
     * </ul>
     *
    ...
    public String generateAccessToken(String eno, List<String> athIds, String bbrC) {
        ...
        List<String> effectiveAthIds =
                (athIds != null && !athIds.isEmpty())
                        ? athIds
     ...
     * Refresh Token 생성
     *
     * <p>사번을 subject로 하는 장기 유효 JWT Refresh Token을 생성합니다. Access Token 만료 시 새로운 Access Token을 발급받는 데
     * 사용합니다.
     *
     * <p>Access Token과 동일한 구조지만 유효시간이 더 깁니다 (기본 7일).
     *
     * <p>{@code jti}(JWT ID) 클레임에 UUID를 넣어 발급마다 토큰 문자열이 고유하도록 보장합니다. JWT 시각 클레임(iat/exp)은 초 단위라,
     * jti 없이는 같은 사용자에게 같은 초 안에 2회 발급 시 완전히 동일한 토큰이 생성되어 TPRMPP_CRTOKM의 HMAC-SHA256 지문 유니크 인덱스
     ...
     *
     * <p>JJWT는 JSON 배열 클레임을 {@code List<String>}으로 역직렬화합니다. 클레임이 없거나 파싱 실패 시 빈 리스트를 반환합니다.
     *
    ...
    public List<String> getAthIdsFromToken(String token) {
        ...
        if (claim != null) {
            log.warn(
                    "[JWT] athIds 클레임 타입 불일치 — 빈 권한 목록 반환: actualType={}",
     ...
     * JWT 토큰에서 사번 추출
     *
     * <p>토큰의 Payload에서 {@code subject} 클레임(사번)을 추출합니다. 토큰 서명 검증 후 Claims를 파싱합니다.
     *
     * <p>주의: 이 메서드는 토큰 검증 없이 직접 호출하면 안 됩니다. 반드시 {@link #validateToken(String)} 호출 후 사용해야 합니다.
     *
     * @param token JWT 토큰 문자열
     ...
     * JWT 토큰 유효성 검증
     *
     * <p>토큰의 서명, 형식, 만료 여부를 검증합니다. {@link JwtAuthenticationFilter}에서 모든 요청의 토큰을 검증하는 데 사용됩니다.
     *
     * <p>검증 항목:
     *
     * <ul>
     *   <li>서명 유효성: secretKey로 서명된 토큰인지 확인
     *   <li>토큰 형식: Header.Payload.Signature 구조 확인
     *   <li>만료 여부: exp 클레임과 현재 시각 비교
     *   <li>지원 여부: JWS(서명된 JWT) 형식인지 확인
     * </ul>
     *
    ...
    public boolean validateToken(String token) {
        try {
            Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token); // 파싱 성공 시 토큰 유효
            return true;
     ...
     *
     * <p>진단·테스트 용도의 조회 메서드입니다. 보안 결정(인증·갱신 허용 여부)은 반드시 {@link #validateToken(String, String,
     * boolean)}로 수행하고 이 메서드에 의존하지 않습니다.
     *
     * @param token JWT 토큰 문자열
     ...
     *
     * <p>서명과 만료를 검증한 뒤 {@code tokenUse} 클레임이 기대 용도와 일치하는지 확인합니다. Claims는 한 번만 파싱하며, 숫자·배열 등 비문자 값이
     * {@code null} 조회와 혼동되어 허용되지 않도록 원본 {@code Object}를 직접 검사합니다.
     *
     * <ul>
     *   <li>{@code tokenUse}가 없는 경우: {@code allowLegacy} 값을 그대로 반환(배포 전 레거시 토큰 한시 허용).
     *   <li>{@code tokenUse}가 문자열이면서 기대 용도와 같을 때만 {@code true}.
     *   <li>문자열이 아닌 값(숫자·배열 등)이나 다른 문자열은 모두 {@code false}.
     *   <li>서명·형식·만료 검증 실패는 토큰 본문 없이 경고 로그 후 {@code false}.
     * </ul>
     *
     * @param token 검증할 JWT 토큰 문자열
