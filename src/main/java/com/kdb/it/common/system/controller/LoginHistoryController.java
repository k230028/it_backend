 ...
 * 로그인 이력 조회 REST 컨트롤러
 *
 * <p>현재 로그인한 사용자의 로그인/로그아웃 이력을 조회합니다.
 *
 * <p>기본 URL: {@code /api/login-history}
 *
 * <p>보안: JWT 토큰 인증 필요. 본인의 이력만 조회 가능합니다. (SecurityContextHolder에서 현재 사용자 사번을 추출하여 조회)
 *
 * <p>이력 유형 ({@code LOGIN_TYPE}):
 *
 * <ul>
 *   <li>{@code 1=성공}: 로그인 성공
 *   <li>{@code LOGIN_FAILURE}: 로그인 실패 (비밀번호 불일치 등)
 *   <li>{@code LOGOUT}: 로그아웃
 * </ul>
 */
...
public class LoginHistoryController {
     ...
     *
     * <p>현재 JWT 토큰으로 인증된 사용자의 로그인 이력을 조회합니다. {@link SecurityContextHolder}에서 사번(eno)을 추출하여 해당 사용자의
     * 이력만 반환합니다.
     *
     * <p>반환 데이터:
     *
     * <ul>
     *   <li>이력 ID
     *   <li>로그인 유형 (1=성공 / LOGIN_FAILURE / LOGOUT)
     *   <li>접속 IP 주소
     *   <li>User-Agent (브라우저/기기 정보)
     *   <li>로그인 시각
     *   <li>실패 사유 (실패한 경우만)
     * </ul>
     *
