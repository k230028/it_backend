 ...
 * 이 컴포넌트는 그 트랜잭션이 완전히 끝난 뒤에만 호출되어야 하며(오케스트레이션은 Task 5 범위), {@code REQUIRES_NEW}로 독립된 새 트랜잭션을 열어
 * 패밀리를 삭제한다. {@link com.kdb.it.domain.log.listener.AuditLogWriter#writeInNewTransaction}와 동일하게
 * {@code flush()}까지 강제해 DELETE 오류를 이 트랜잭션 경계 안에서 즉시 확정한다.
 *
