 ...
 *
 * <p>{@link PostConstruct}로 스프링 컨텍스트 초기화 직후 실행되며, 운영에 필요한 비밀값(DB 비밀번호, JWT 시크릿, 토큰 지문 시크릿)이 빈값이면 즉시
 * 기동을 중단합니다. 단, {@code application.properties}에 기본값이 남아 있으면 환경변수 미설정도 통과하므로 운영 프로파일에서는 기본값 제거 또는 별도
 * 검증이 필요합니다.
 *
 * <p>검증 대상:
 *
...
public class EnvironmentValidator {
    ...
    public void validate() {
        ...
        checkMinimumUtf8Bytes("security.token-fingerprint-secret", "TOKEN_FINGERPRINT_SECRET", 32);
