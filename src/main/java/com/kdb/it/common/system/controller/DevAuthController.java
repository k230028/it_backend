 ...
 * 개발 편의용 사용자 전환 컨트롤러
 *
 * <p>비밀번호 입력 없이 임의의 사용자로 세션을 전환할 수 있는 개발 전용 API입니다. 협의회 등 권한별 화면을 빠르게 검증하기 위해 도입했습니다.
 *
 * <p>활성화 조건: {@code app.dev.user-switch.enabled=true}. 기본값은 비활성이며 로컬/개발 프로파일에서만 명시적으로 켭니다.
 *
 * <p>제공 엔드포인트:
 *
 * <ul>
 *   <li>{@code GET /api/auth/dev/users} — 전환 가능한 사용자 목록(ID/부서명/팀명/이름)
 *   <li>{@code POST /api/auth/dev/switch-user} — 선택한 사번으로 JWT 쿠키 재발급
 * </ul>
 */
@RestController
...
public class DevAuthController {
     ...
     *
     * <p>이름순 정렬로 전체 활성 사용자(미삭제 사용자)를 반환합니다. {@link UserDto.ListResponse}를 재사용해 ID/부서명/팀명/이름 등을 한 번에
     * 전달합니다.
     *
     ...
     *
     * <p>{@link AuthService#issueDevSwitchTokens(String)}을 호출해 토큰을 재발급하고 httpOnly 쿠키
     * 3종(Access/Refresh/it-portal-user)을 응답에 세팅합니다. 프론트엔드는 응답 body의 사용자 정보로 Pinia 상태를 갱신합니다.
     *
     ...
     *
     * <p>{@link CuserI#getBbrNm()}은 조직 연관관계를 통해 조회되므로 EntityGraph로 즉시 로딩된 조직 정보가 필요합니다.
     */
