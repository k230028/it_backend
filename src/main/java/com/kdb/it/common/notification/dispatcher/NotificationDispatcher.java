 ...
 * 알림 외부 발송 전략 SPI
 *
 * <p>본 페이즈에서는 인앱(INAPP) 적재만 즉시 처리한다. 이메일·SMS·알림톡 등 외부 채널 어댑터는 본 인터페이스를 구현하여 Phase 2에서 추가한다.
 */
public interface NotificationDispatcher {
     ...
     *
     * <p>구현체는 채널 발송만 담당하고 엔티티 상태는 변경하지 않는다. 예상 가능한 발송 실패는 예외 대신 실패 결과로 반환한다.
     *
