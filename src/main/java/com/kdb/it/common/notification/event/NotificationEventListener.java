 ...
 *
 * <ul>
 *   <li>{@link NotificationEvent} — 명시적 발송 이벤트 (결재요청·멘션 등 도메인 호출자가 발행)
 *   <li>{@link ApprovalCompletedEvent} — 결재 완료/반려 이벤트 (신청자에게 결재결과 알림으로 변환)
 *   <li>{@link ApprovalRecalledEvent} — 결재회수 이벤트 (신청자 및 기승인 중간결재자에게 결재회수 알림)
 * </ul>
 *
 * <p>핸들러는 모두 {@link TransactionPhase#AFTER_COMMIT}이며, 발송 실패는 원본 트랜잭션에 영향을 주지 않도록 catch 후 warn 로깅한다.
 */
@Slf4j
@Component
...
public class NotificationEventListener {
    ...
    public void onApprovalCompleted(ApprovalCompletedEvent event) {
        try {
            ...
            if (capplm == null) {
                log.warn(
                        "Approval result notification skipped: capplm not found. apfMngNo={}",
                        event.apfMngNo());
                return;
            }
    ...
    public void onApprovalRecalled(ApprovalRecalledEvent event) {
        try {
            ...
            if (capplm == null) {
                log.warn(
                        "Approval recall notification skipped: capplm not found. apfMngNo={}",
                        event.apfMngNo());
                return;
            }
            ...
            if (capplm.getDcdReqUsid() != null
                    && !capplm.getDcdReqUsid().equals(event.recallerEno())) {
                enqueueAndDispatch(
                        NotificationEvent.builder()
                                .recipientEno(capplm.getDcdReqUsid())
                ...
                for (String eno : event.approvedMiddleApproverEnos()) {
                    ...
                    enqueueAndDispatch(
                            NotificationEvent.builder()
                                    ...
                                    .infmMsgCone(
                                            NotificationMessageFormatter.abbreviate(
                                                    "귀하가 결재한 신청서가 회수되었습니다: " + apfNm, 4000))
                                    .infmRcdUrl(linkUrl)
                                    .build());
                }
            }
