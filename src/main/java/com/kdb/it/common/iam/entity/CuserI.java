 ...
 * 사용자(직원) 정보 엔티티
 *
 * <p>DB 테이블: {@code TPRMPP_CUSERI}
 *
 * <p>시스템 사용자(직원)의 기본 정보와 인증 정보를 관리합니다. Spring Security의 {@code UserDetails} 소스로 활용됩니다.
 *
 * <p>연관 관계:
 *
 * <ul>
 *   <li>{@link CorgnI}와 ManyToOne 관계 ({@code BBR_C} → {@code PRLM_OGZ_C_CONE})
 *   <li>조인 컬럼 {@code insertable=false, updatable=false}: 읽기 전용 조인 (BBR_C 자체로 저장)
 * </ul>
 */
@Entity // JPA 엔티티로 등록
...
public class CuserI extends BaseEntity {
    ...
    /** 행번(사번): 기본키. 직원을 고유하게 식별하는 사번 로그인 ID로도 사용되며, JWT 토큰의 subject에 저장됩니다. */
    @Id
    @Column(name = "ENO", nullable = false, length = 32, comment = "사원번호")
    private String eno;

    /**
     * 사용자암호화패스워드: SHA-256으로 암호화된 비밀번호 {@link CustomPasswordEncoder}로 암호화하여 저장합니다. 최대 64자 (SHA-256 +
     * Base64 인코딩 결과)
     */
    @Column(
            name = "USR_ECY_PWD",
            length = 64,
            comment = "사용자암호화패스워드 (물리컬럼 USR_ECY_PWD=메타표준 사용자암호화비밀번호)")
    private String usrEcyPwd;

    /** 부서코드: 직원이 소속된 부점의 코드 {@link CorgnI}의 {@code PRLM_OGZ_C_CONE}과 조인 키로 사용 */
     ...
     *
     * <p>{@code insertable=false, updatable=false}: 이 컬럼을 직접 INSERT/UPDATE하지 않고 {@code bbrC} 필드를
     * 통해서만 관리 (읽기 전용 조인)
     *
     * <p>{@code FetchType.LAZY}: 실제 접근 시에만 조회 (N+1 방지를 위해 EntityGraph 사용)
     */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "BBR_C",
            referencedColumnName = "PRLM_OGZ_C_CONE",
            insertable = false,
            updatable = false)
     ...
     * 부점명 조회 편의 메서드
     *
     * <p>{@link CorgnI} 연관 관계에서 부점명을 반환합니다. 조직 정보가 없는 경우 null을 반환합니다.
     *
     ...
     * 상위조직코드 조회 편의 메서드
     *
     * <p>{@link CorgnI} 연관 관계에서 상위조직코드를 반환합니다. 조직 정보가 없는 경우 null을 반환합니다.
     *
     ...
     * 상위조직명 조회 편의 메서드
     *
     * <p>{@link CorgnI} 연관 관계의 상위조직({@code parentOrganization}) 부점명을 반환합니다. 조직 정보 또는 상위조직 정보가 없는 경우
     * null을 반환합니다.
     *
     ...
     *
     * <p>부모 클래스({@link BaseEntity})의 {@code prePersist()}를 호출한 후 CuserI 엔티티 고유의 초기화 로직을 추가로 수행합니다.
     */
    @Override
    ...
    /**
     * 사용자 기본정보 업데이트 (Dirty Checking 활용) 비밀번호는 별도 updatePassword()로 처리합니다.
     *
     * @param usrNm 사용자명
     * @param ptCNm 직위명
     * @param temC 팀코드
     ...
     * 비밀번호 변경 메서드
     *
     * <p>암호화된 비밀번호로 업데이트합니다. 반드시 {@link CustomPasswordEncoder}로 암호화한 값을 전달해야 합니다.
     *
