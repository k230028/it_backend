 ...
 * 조직(부점) 정보 엔티티
 *
 * <p>DB 테이블: {@code TPRMPP_CORGNI}
 *
 * <p>회사 내 부점(부서/팀)의 조직 정보를 관리합니다. 계층형 구조로 상위 조직과의 관계를 {@code PRLM_HRK_OGZ_C_CONE}으로 표현합니다.
 *
 * <p>연관 관계:
 *
 * <ul>
 *   <li>{@link CuserI}와 ManyToOne 관계 ({@code BBR_C} → {@code PRLM_OGZ_C_CONE})
 * </ul>
 *
 * <p>조직 코드는 {@link CuserI#bbrC} 필드와 조인하여 사용자의 소속 부점명을 조회합니다.
 */
@Entity // JPA 엔티티로 등록
...
public class CorgnI extends BaseEntity {
    ...
    /**
     * 조직코드: 기본키. 조직을 고유하게 식별하는 코드 (예: "001", "HR001" 등 최대 100자) {@link CuserI}의 {@code BBR_C} 컬럼과
     * 조인 대상
     */
    @Id
    ...
    /** 인사상위조직코드내용: 이 조직의 부모 조직 코드 최상위 조직의 경우 null이거나 자기 자신을 가리킴 조직 트리(계층 구조) 구성에 사용 */
     ...
     *
     * <p>{@code insertable=false, updatable=false}: 이 컬럼을 직접 INSERT/UPDATE하지 않고 {@code
     * prlmHrkOgzCCone} 필드를 통해서만 관리 (읽기 전용 조인)
     *
     * <p>{@code FetchType.LAZY}: 실제 접근 시에만 조회 (EntityGraph로 즉시 로딩 가능)
     */
