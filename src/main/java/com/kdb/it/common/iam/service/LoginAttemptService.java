...
public class LoginAttemptService {
     ...
     * 계정 잠금 여부를 확인합니다.
     *
     * <p>직전 {@value WINDOW_MINUTES}분 이내 로그인 실패 횟수가 {@value MAX_FAILURES}회 이상이면
     * {@link CustomGeneralException}을 발생시켜 인증을 차단합니다.</p>
     *
    ...
    public void checkLocked(String eno) {
        ...
        if (failureCount >= MAX_FAILURES) {
            throw new CustomGeneralException(
                    "계정 잠금: " + WINDOW_MINUTES + "분 내 로그인 실패가 " + MAX_FAILURES + "회 이상입니다. 잠시 후 다시 시도하세요.");
        }
    }
}
