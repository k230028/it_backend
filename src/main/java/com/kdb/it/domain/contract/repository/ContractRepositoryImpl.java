...
public class ContractRepositoryImpl implements ContractRepositoryCustom {
    ...
    public List<ContractDto.ListItem> search(String stsTc, String bgPrnTc, String cncdRfrNo, String bbrC) {
        ...
        if (StringUtils.hasText(bbrC)) {
            where.and(Expressions.anyOf(
                    Expressions.allOf(ct.bgPrnTc.eq("100"), p.svnDpmC.eq(bbrC)),
                    Expressions.allOf(ct.bgPrnTc.eq("200"), c.costSvnDpmC.eq(bbrC))
            ));
        }
    ...
    public Optional<ContractTargetRow> findCurrentWithTargetName(String docNo) {
        ...
        Tuple row = queryFactory
                .select(ct,
                        new CaseBuilder()
                                .when(ct.bgPrnTc.eq("100")).then(p.abusNm)
                                .when(ct.bgPrnTc.eq("200")).then(c.cttNm)
