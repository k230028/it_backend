 ...
 *
 * <p>대상명(tgtNm)은 대상구분(ioeC)에 따라 Bprojm 또는 Bcostm 두 곳에서 가져와야 하므로 JOIN 구조가 복잡합니다. MVP에서는 목록에 대상명을
 * 포함하지 않고, 단건 조회(get) 시 서비스 계층에서 해석합니다.
 *
 * <p>bbrC 필터: 대상구분(ioeC)에 따라 조건부 LEFT JOIN으로 부서 필터를 적용합니다. 사업(100)은 Bprojm.svnDpmC, 전산업무비(200)은
 * Bcostm.costSvnDpmC와 비교합니다.
 */
...
public class ContractRepositoryImpl implements ContractRepositoryCustom {
     ...
     * 입찰계약 목록 동적 검색.
     *
     * <p>stsTc·ioeC·cncdRfrNo 모두 null/빈값이면 전체 조회(관리자 뷰).
     *
     ...
     */
    @Override
    public List<ContractDto.ListItem> search(
            String stsTc, String ioeC, String cncdRfrNo, String bbrC) {
        ...
        if (StringUtils.hasText(ioeC)) where.and(ct.ioeC.eq(ioeC));
        ...
        if (StringUtils.hasText(bbrC)) {
            where.and(
                    Expressions.anyOf(
                            Expressions.allOf(ct.ioeC.eq("100"), p.svnDpmC.eq(bbrC)),
                            Expressions.allOf(ct.ioeC.eq("200"), c.costSvnDpmC.eq(bbrC))));
        }
        ...
        return queryFactory
                ...
                .from(ct)
                .leftJoin(p)
                .on(p.abusMngNo.eq(ct.cncdRfrNo).and(p.lstYn.eq("Y")).and(p.delYn.eq("N")))
                .leftJoin(c)
                .on(c.costBgNo.eq(ct.cncdRfrNo).and(c.lstYn.eq("Y")).and(c.delYn.eq("N")))
                .where(where)
     ...
     * 현재 유효 마스터 + 대상명 단일 조회.
     *
     * <p>마스터와 대상명을 한 번의 쿼리(Tuple)로 가져옵니다. 대상명은 대상구분(ioeC)에 따라 Bprojm(100=사업, ABUS_NM) 또는
     * Bcostm(200=전산업무비, CTT_NM)을 cncdRfrNo 키로 LEFT JOIN하여 CASE 식으로 분기합니다. 대상 레코드가 없으면 LEFT JOIN 특성상
     * 대상명은 null입니다.
     *
     * @param docNo 문서관리번호
     ...
     */
    @Override
    public Optional<ContractTargetRow> findCurrentWithTargetName(String docNo) {
        QBcontm ct = QBcontm.bcontm;
        QBprojm p = QBprojm.bprojm;
        QBcostm c = QBcostm.bcostm;
        Tuple row =
        ...
        return Optional.of(
                new ContractTargetRow(row.get(0, Bcontm.class), row.get(1, String.class)));
    }
}
