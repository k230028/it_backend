...
public interface EstimateLineRepository extends JpaRepository<Besttm, BesttmId> {
     ...
     *
     * <p>운영 PK는 (문서번호+버전+개선의견일련번호) 3컬럼이라 (팀+비목) 업무 키의 유일성을 DB가 더 이상 보장하지 않는다. 동일 키를 가진 물리 행이 2건 이상
     * 존재할 때 {@code EstimateService.saveLines()}는 "먼저 나온 (=낮은 일련번호) 행을 대표로 남긴다"는 규칙으로 병합하므로, 이 조회가
     * 명시적으로 오름차순 정렬을 보장하지 않으면 그 규칙이 실제로 성립하지 않는다. 정렬 없는 파생 쿼리는 Oracle에서 행 순서를 보장하지 않는다.
     *
     * @param rqmBgReqDocNo 소요예산요청문서번호
     * @param docVrsSno 문서버전일련번호
