 ...
 *
 * <p>Bprojm(p) LEFT JOIN Bestim(e) — 개발비 품목이 있는 소속 부서 사업을 기준으로 목록을 구성하고,
 * 진행 중인 산정 문서가 있으면 문서 정보를 함께 가져옵니다.</p>
 */
...
public class EstimateRepositoryImpl implements EstimateRepositoryCustom {
    ...
    public List<EstimateDto.ListItem> search(String stsTc, String cncdRfrNo, String bbrC) {
        ...
        where.and(JPAExpressions
                .selectOne()
                .from(devItem)
                .where(devItem.abusMngNo.eq(p.abusMngNo)
                        .and(devItem.fntTbCrySno.eq(p.sno))
                        .and(devItem.lstYn.eq("Y"))
                        .and(devItem.delYn.eq("N"))
                        .and(devItem.ioeC.in(JPAExpressions
                                .select(code.cdva)
                                .from(code)
                                .where(code.cId.eq("IOE_C")
                                        .and(code.cTp.eq("IOE_DVC"))
                                        .and(code.cdva.ne(DEV_CONSULTING_IOE_C))
                                        .and(code.delYn.eq("N"))))))
                .exists());
        if (StringUtils.hasText(stsTc))     { where.and(e.stsTc.eq(stsTc)); }
        if (StringUtils.hasText(cncdRfrNo)) { where.and(p.abusMngNo.eq(cncdRfrNo)); }
        ...
        return queryFactory
                ...
                .from(p)
                .leftJoin(e).on(e.cncdRfrNo.eq(p.abusMngNo).and(e.lstYn.eq("Y")).and(e.delYn.eq("N")))
                .leftJoin(item).on(item.abusMngNo.eq(p.abusMngNo)
                        .and(item.fntTbCrySno.eq(p.sno))
                        .and(item.lstYn.eq("Y"))
                        .and(item.delYn.eq("N")))
                .where(where)
                .groupBy(e.rqmBgReqDocNo, e.docVrsSno, p.abusMngNo, p.abusNm, p.sttDtm,
                        p.endDtm, p.svnDpmC, p.svnDpmNm, e.stsTc, e.fstEnrUsid, e.fstEnrDtm)
