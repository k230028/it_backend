...
public class PaymentRepositoryImpl implements PaymentRepositoryCustom {
     ...
     */
    @Override
    public List<PaymentDto.ListItem> search(
        ...
        if (StringUtils.hasText(stsTc)) {
            where.and(pm.stsTc.eq(stsTc));
        }
        if (StringUtils.hasText(ioeC)) {
            where.and(pm.ioeC.eq(ioeC));
        }
        if (StringUtils.hasText(cncdRfrNo)) {
            where.and(pm.cncdRfrNo.eq(cncdRfrNo));
        }
        ...
        return queryFactory
                ...
                .from(pm)
                .leftJoin(p)
                .on(p.abusMngNo.eq(pm.cncdRfrNo).and(p.lstYn.eq("Y")).and(p.delYn.eq("N")))
                .leftJoin(c)
                .on(c.costBgNo.eq(pm.cncdRfrNo).and(c.lstYn.eq("Y")).and(c.delYn.eq("N")))
                .where(where)
     ...
     * 대상명은 null입니다. 회차별 지급 명세(1:N)는 본 쿼리에 포함하지 않고 서비스에서 별도 조회합니다.
     *
     * @param docNo 문서관리번호
    ...
    public Optional<PaymentTargetRow> findCurrentWithTargetName(String docNo) {
        ...
        return Optional.of(
                new PaymentTargetRow(row.get(0, Bpaymm.class), row.get(1, String.class)));
    }
}
