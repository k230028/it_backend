 ...
 *
 * <p>결재완료 필터링 + 비목 접두어 매칭 등 복잡한 동적 쿼리를 정의합니다. 기존 {@code ProjectRepositoryImpl}, {@code
 * CostRepositoryImpl}의 CAPPLA+CAPPLM 서브쿼리 패턴을 재사용합니다. // Design Ref: §4.6 — BbugtmRepositoryCustom
 * (QueryDSL)
 */
public interface BbugtmRepositoryCustom {
     ...
     *
     * <p>CCODEM 마이그레이션(V003) 이후 BCOSTM.IOE_C는 단축 cdva 값("001" 등)을 저장하므로 LIKE 접두어 매칭 대신 IN 조건을
     * 사용합니다.
     *
     ...
     *
     * <p>CCODEM 마이그레이션(V003) 이후 BITEMM.IOE_C는 단축 cdva 값을 저장하므로 LIKE 접두어 매칭 대신 IN 조건을 사용합니다.
     *
     ...
     *
     * <p>정보화사업 편성은 품목(BITEMM) 단위로 저장되므로(ORC_TB='BITEMM'), BBUGTM을 BITEMM과 JOIN하여 BITEMM.PRJ_MNG_NO
     * 기준으로 SUM(DUP_BG)를 집계합니다.
     *
     ...
     *
     * <p>TPRMPP_BBUGTM에서 ORC_TB='BCOSTM' 조건으로 itMngcNo별 SUM(DUP_BG)를 집계합니다.
     *
     ...
     *
     * <p>BBUGTM JOIN BITEMM, gclDtt ∈ assetGclDttCodes(공통코드 IOE_CPIT)인 품목의 DUP_BG를 PRJ_MNG_NO 기준으로
     * 집계합니다.
     *
     ...
     * @return prjMngNo → SUM(dupBgAmt) 맵
     */
    Map<String, BigDecimal> sumAssetDupBgByPrjMngNos(
            List<String> prjMngNos, String bgYy, Set<String> assetGclDttCodes);

    /**
     * 정보화사업별 일반관리비 편성예산(DUP_BG) 합계 조회
     *
     * <p>BBUGTM JOIN BITEMM, gclDtt ∈ costGclDttCodes(공통코드 IOE_IDR/SEVS/XPN/LEAFE)인 품목의 DUP_BG를
     ...
     * @return prjMngNo → SUM(dupBgAmt) 맵
     */
    Map<String, BigDecimal> sumCostDupBgByPrjMngNos(
            List<String> prjMngNos, String bgYy, Set<String> costGclDttCodes);

    /**
     * 비목 ioeCValues 집합별 결재완료 요청금액 합계 조회
     *
     * <p>편성비목 조회(API-01) 시 각 비목별 결재완료 요청금액을 집계합니다. BCOSTM.IT_MNGC_BG + BITEMM.GCL_AMT를 ioeCValues
     * 기준으로 SUM합니다.
     *
     * @param ioeCValues 해당 편성비목에 속하는 IOE cdva 값 집합
     * @param bgYy 예산연도
