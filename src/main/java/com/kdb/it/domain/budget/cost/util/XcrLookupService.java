 ...
 *
 * <p>
 * REQUIREMENTS.md R3.7 / CONTEXT.md 결정 E 에 따라, 외화 저장 시 클라이언트가 보낸
 * {@code xcr} 값을 신뢰하지 않고 서버가 {@code Ccodem}(공통코드마스터)을 단일 진실 원천으로
 * 조회해 환율을 결정한다.
 * </p>
 *
 * <p>조회 규약 (Ccodem):</p>
 ...
 *
 * <p>
 * 적용 지점은 {@code CostService} (Bcostm + Btermm), {@code ProjectService} (Bitemm),
 * {@code BudgetWorkService} (편성 재집계) 의 외화 처리 경로다.
 * </p>
 */
@Service
...
public class XcrLookupService {
    ...
    public BigDecimal resolveXcr(String curC, LocalDate baseDate) {
        ...
        Ccodem ccodem = codeRepository.findByCIdAndCdvaWithValidDate(CUR_C_ID, curC, target)
