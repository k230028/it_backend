 ...
 *
 * <p>테이블: {@code TPRMPP_BRIVGM}
 *
 * <p>기본 CRUD는 {@link JpaRepository}를 통해 제공되며, 본 인터페이스는 파생 쿼리 메서드만 선언합니다. 복잡한 동적 쿼리가 필요해지면 별도의
 * {@code BrivgmRepositoryCustom} + QueryDSL 구현체로 분리합니다.
 */
public interface BrivgmRepository extends JpaRepository<Brivgm, Long> {
     ...
     *
     * <p>URL 상의 {@code docMngNo}와 실제 코멘트의 소속 문서가 일치하는지 함께 검증합니다. 이를 통해 다른 문서의 {@code ipmOpnnSno}를
     * 이용한 교차 접근을 차단합니다.
     *
     * @param ipmOpnnSno 의견일련번호
     * @param docMngNo 문서관리번호 (예: DOC-2026-0001)
     ...
     * @return 조건에 해당하는 검토의견 (없으면 {@link Optional#empty()})
     */
    Optional<Brivgm> findByIpmOpnnSnoAndDocMngNoAndDelYn(
            Long ipmOpnnSno, String docMngNo, String delYn);
}
