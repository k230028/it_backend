 ...
 * 요구사항 정의서 엔티티
 *
 * <p>DB 테이블: {@code TPRMPP_BRDOCM}
 *
 * <p>IT 프로젝트의 요구사항 정의서를 관리합니다.
 *
 * <p>관리번호 형식: {@code DOC-{연도}-{4자리 시퀀스}} (예: {@code DOC-2026-0001})
 */
...
public class Brdocm extends BaseEntity {
     ...
     *
     * <p>물리 컬럼은 Oracle {@code NUMBER(9,0)}(정수)입니다. 소수 버전(0.01 단위)을 그대로 저장하면 절삭되어 PK가 충돌하므로, 화면 소수
     * 버전 × 100을 정수로 저장합니다. (예: 화면 0.01 → 저장 1, 화면 1.00 → 저장 100). 화면 변환은 {@link
     ...
     *
     * <p>신규 생성 및 새 버전 생성 시 작성자(현재 로그인 사용자) 소속 부서·팀 코드로 채우고, 조직명은 같은 시점의 CORGNI 조회 결과(스냅샷)로 저장합니다.
     * 변경 로그 스냅샷이 값을 복사하도록 반드시 INSERT 이전(save 호출 전)에 호출합니다.
     *
     ...
     * 새 버전 엔티티 생성 메서드
     *
     * <p>현재 엔티티의 업무 필드({@code reqTtl}, {@code redtConeInf}, {@code reqDttNo}, {@code bzDttNm},
     * {@code rvwFsgTlmDt})를 복제하여 지정된 버전({@code nextVrs})의 새 {@link Brdocm} 인스턴스를 반환합니다.
     *
     * <p>{@link com.kdb.it.domain.entity.BaseEntity#prePersist()} 가 {@code delYn}, {@code guid},
     * {@code guidPrgSno} 및 JPA Auditing 필드({@code fstEnrDtm}, {@code fstEnrUsid} 등)를 INSERT 시점에 자동
     * 설정하므로, 이 메서드에서는 해당 필드들을 별도로 지정하지 않습니다.
     *
