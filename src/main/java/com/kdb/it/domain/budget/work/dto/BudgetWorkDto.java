 ...
 * 예산 작업 관련 DTO 클래스 모음
 *
 * <p>예산 편성률 적용(TPRMPP_BBUGTM)의 요청/응답 DTO를 정적 중첩 클래스(Static Nested Class) 형태로 관리합니다.
 *
 * <p>포함된 DTO:
 *
 * <ul>
 *   <li>{@link ApplyRequest}: 편성률 일괄 적용 요청
 *   <li>{@link RateItem}: 개별 비목 편성률
 *   <li>{@link IoeCategoryResponse}: 편성비목 목록 조회 응답
 *   <li>{@link SummaryResponse}: 편성 결과 요약 응답
 *   <li>{@link SummaryItem}: 비목별 요약 항목
 *   <li>{@link SummaryTotals}: 합계
 *   <li>{@link ApplyResponse}: 편성률 적용 결과 응답
 * </ul>
 *
...
public class BudgetWorkDto {
    ...
    public record ProjectSummaryItem(
            ...
            @Schema(description = "편성금액 합계") BigDecimal dupAmount,
            @Schema(description = "비목별 금액 맵 (key: ioePrefix, value: {request, dup})")
                    Map<String, CategoryAmount> categoryAmounts) {}
