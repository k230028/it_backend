 ...
 * 전산관리비(IT 관리비) 서비스
 *
 * <p>전산관리비(TPRMPP_BCOSTM) 엔티티의 생성, 조회, 수정, 삭제(Soft Delete) 비즈니스 로직을 처리합니다.
 *
 * <p>복합키 구조:
 *
 * <ul>
 *   <li>{@code IT_MNGC_NO} (전산관리비관리번호): 논리적 식별자
 *   <li>{@code BG_SNO} (예산일련번호): 동일 관리번호 내의 이력 순번
 * </ul>
 *
 * <p>조회/수정/삭제는 {@code IT_MNGC_NO}를 기준으로 하며, {@code LST_YN='Y'}인 항목(최신 이력)을 대상으로 처리합니다.
 *
 * <p>Soft Delete 패턴: {@code DEL_YN='Y'}로 변경하여 논리 삭제합니다.
 *
...
public class CostService {
     ...
     * 특정 전산관리비 단건 조회
     *
     * <p>{@code IT_MNGC_NO}로 삭제되지 않은({@code DEL_YN='N'}) 항목을 조회합니다. 동일 관리번호에 여러 이력(SNO)이 있으면 {@link
     * CostRepresentativeSelector#pick(List)}이 최신·활성 조건(LST_YN='Y' 우선, BG_SNO 내림차순)으로 대표 행을 결정적으로
     * 선택합니다.
     *
     ...
     * 전체 전산관리비 목록 조회
     *
     * <p>삭제되지 않은({@code DEL_YN='N'}) 모든 전산관리비를 조회하여 DTO 목록으로 변환하여 반환합니다.
     *
     ...
     * 검색 조건으로 전산관리비 목록 조회
     *
     * <p>{@link CostDto.SearchCondition}의 조건이 모두 비어있으면 전체 조회({@link #getCostList()})와 동일합니다.
     *
     * <p>{@code apfSts} 필터 처리:
     *
     * <ul>
     *   <li>{@code "none"}: 신청서가 없는 전산관리비만 조회 (CAPPLA 연결 없음)
     *   <li>그 외 값: 최신 신청서의 결재상태가 해당 값인 전산관리비만 조회
     *   <li>null/미입력: 결재상태 필터 없음
     * </ul>
     *
    ...
    public String createCost(CostDto.CreateRequest request) {
        ...
        bcostm.assignSvnOrgNames(
                orgNameResolver.resolveName(bcostm.getCostSvnDpmC()), orgSnapshot.svnTemNm());
        ...
        if (request.getTerminals() != null && !request.getTerminals().isEmpty()) {
            for (CostDto.TerminalDto tDto : request.getTerminals()) {
                ...
                BigDecimal[] tReconciled =
                        BudgetAmountCalculator.reconcileAmount(
                                tDto.getFcAmt(),
                                tDto.getTermRqmBgAmt(),
                                tDto.getCurC(),
                                tDto.getXcr());
     ...
     * 전산관리비 수정
     *
     * <p>{@link CostRepresentativeSelector#pick(List)} 규칙으로 {@code LST_YN='Y'}인 이력을 우선하고, 같으면
     * {@code BG_SNO} 내림차순으로 대표 행을 결정해 수정합니다.
     *
     * <p>JPA Dirty Checking: 조회된 엔티티의 필드를 변경하면 트랜잭션 종료 시 자동으로 UPDATE 쿼리가 실행됩니다.
     *
    ...
    public String updateCost(String itMngcNo, CostDto.UpdateRequest request) {
        ...
        Map<String, Btermm> existingByPk =
                existingTerminals.stream()
                        .collect(
                                Collectors.toMap(
                                        t -> terminalPk(t.getTmnMngNo(), t.getSno()),
                                        t -> t,
                                        (a, b) -> a));

        List<CostDto.TerminalDto> requestTerminals =
        ...
        for (CostDto.TerminalDto tDto : requestTerminals) {
            ...
            Btermm existing =
                    (tDto.getTmnMngNo() != null && tDto.getSno() != null)
            ...
            if (existing != null) {
                ...
                existing.update(
                        ...
                        tDto.getCurC(),
                        tDto.getXcr(),
                        DateFormatUtil.toYmd8(tDto.getXcrBseDt()),
                        tDto.getDfrCleC(),
                        tDto.getIndRsn(),
                        tDto.getCgprId(),
                        tDto.getTermSvnTemC(),
     ...
     *
     * <p>주관팀명(SVN_TEM_NM)/상위조직명(PRLM_HRK_OGZ_C_CONE) 스냅샷용. 팀코드(SVN_TEM_C)는 조직마스터(CORGNI)에 등재되지 않고
     * 상위조직도 코드/명 매핑이 달라 CORGNI 조회로는 명칭을 얻기 어렵다. 따라서 담당자(CUSERI) 레코드의 팀명·상위조직명을 그대로 저장 시점 스냅샷으로
     * 사용한다(프로젝트 저장 로직과 동일). 담당팀·담당자는 화면에서 동일 직원 기준으로 함께 설정되므로 담당자 기준 값이 화면 선택과 일치한다. 담당자 미지정(사번
     * null/공백)이거나 CUSERI 미조회 시 두 값 모두 {@code null}이다(대상 컬럼 nullable).
     *
     ...
     *
     * <p>물리 컬럼 {@code SVN_TEM_C}(주관팀코드)에는 담당자의 팀코드({@code CUSERI.TEM_C}), {@code
     * SVN_DPM_C}(주관부서코드)에는 담당자의 부서코드({@code CUSERI.BBR_C})를 채워, 프론트가 넘긴 값이나 과거 잘못 저장된 값(부서코드가 팀코드
     * 컬럼에 유입되던 문제)을 저장 시점에 교정한다. 단말기 화면에서 팀/부서는 담당자 선택과 항상 함께 설정되므로 담당자 기준 값이 화면 의도와 일치한다.
     *
     * <p>담당자 사번(cgprId)이 비었거나 CUSERI 미조회인 행은 DTO 값을 그대로 둔다(대상 컬럼 nullable). 담당자 사번 배치
     * 조회(findByEnoIn)로 N+1을 방지한다.
     *
    ...
    private void applyTerminalOrgCodes(List<CostDto.TerminalDto> terminals) {
        ...
        Map<String, CuserI> userByEno =
                cuserIRepository.findByEnoIn(enos).stream()
                        .collect(
                                Collectors.toMap(user -> user.getEno(), user -> user, (a, b) -> a));
     ...
     *
     * <p>{@code IT_MNGC_NO}에 해당하는 모든 이력(SNO)에 대해 {@code DEL_YN='Y'}로 설정합니다.
     *
     * <p>물리 삭제(DELETE)를 수행하지 않으며, 논리적으로 삭제 처리합니다.
     *
    ...
    public void deleteCost(String itMngcNo) {
        ...
        for (Bcostm cost : costs) {
            cost.delete();
            terminalsByKey
                    .getOrDefault(cost.getCostBgNo() + "_" + cost.getBgSno(), List.of())
     ...
     * 전산관리비 일괄 조회
     *
     * <p>여러 관리번호를 한 번에 조회합니다. 존재하지 않는 항목은 결과에서 제외합니다.
     *
    ...
    public CostDto.BulkResponse getCostsByIds(CostDto.BulkGetRequest request) {
        ...
        if (bseYy != null && !bseYy.isBlank() && !responses.isEmpty()) {
            List<String> costBgNos = responses.stream().map(value -> value.getCostBgNo()).toList();
            Map<String, BigDecimal> dupBgMap =
                    bbugtmRepository.sumDupBgByItMngcNos(costBgNos, bseYy);
            // 전산업무비는 ioeC가 IOE_CPIT이면 자본예산, 나머지면 일반관리비 단일 분류
            responses.forEach(
                    r -> {
                        BigDecimal dupBgAmt =
                                dupBgMap.getOrDefault(r.getCostBgNo(), BigDecimal.ZERO);
                        r.setDupBgAmt(dupBgAmt);
                        boolean isAsset =
                                r.getAssetBg() != null
                                        && r.getAssetBg().compareTo(BigDecimal.ZERO) > 0;
     ...
     *
     * <p>전산관리비관리번호와 순번으로 연결된 신청서(CAPPLA) 중 가장 최신 신청서를 조회하여 응답 DTO에 신청관리번호({@code apfMngNo})와
     * 결재상태({@code apfSts})를 설정합니다.
     *
     * <p>조회 기준:
     *
     * <ul>
     *   <li>{@code ORC_TB_CD = 'BCOSTM'}: 전산관리비 원본 테이블 코드
     *   <li>{@code ORC_PK_VL = itMngcNo}: 전산관리비관리번호
     *   <li>{@code ORC_SNO_VL = itMngcSno}: 전산관리비일련번호
     *   <li>최신순 정렬 ({@code APF_REL_SNO DESC})
     * </ul>
     *
    ...
    private void setApplicationInfo(CostDto.Response response, String costBgNo, Integer bgSno) {
        List<com.kdb.it.common.approval.repository.ApplicationMapRepository.ApplicationMapView>
                capplas =
                        capplaRepository
        ...
        if (!capplas.isEmpty()) {
            com.kdb.it.common.approval.repository.ApplicationMapRepository.ApplicationMapView
                    cappla = capplas.get(0);
                                ...
                                response.setApfSts(
                                        capplm.getItPtlApfPrgStsC() == null
                                                ...
                                                : com.kdb.it.common.approval.domain.ApprovalStatus
                                                        .ofCode(capplm.getItPtlApfPrgStsC())
                                                        .label());
                                List<
                                                com.kdb.it.common.approval.repository
                                                        .ApproverRepository.ApproverReadView>
                                        decisions =
                                                cdecimRepository
                                                        .findReadViewsByDcdMngNoOrderByDcrSqnSnoAsc(
                                                                cappla.getApfDcmNo());
                                response.setApplicationInfo(
                                        ApplicationInfoDto.fromReadViews(capplm, decisions));
                            });
        }
    }
     ...
     *
     * <p>비목코드(ioeC)를 공통코드에서 조회하여 코드값구분(cttTp) 기준으로 분류합니다.
     *
     * <ul>
     *   <li>자본예산: cttTp가 IOE_DVC/IOE_HW/IOE_SW인 경우 → assetBg = itMngcBgAmt, costBg = 0
     *   <li>일반관리비: cttTp가 IOE_IDR, IOE_SEVS, IOE_XPN, IOE_LEAFE인 경우 → assetBg = 0, costBg =
     *       itMngcBgAmt
     * </ul>
     *
    ...
    private void setBudgetCategory(CostDto.Response response) {
        ...
        Optional<Ccodem> codeOpt =
                ccodemRepository.findByCIdWithValidDate(CommonCodeGroups.IOE, null).stream()
     ...
     *
     * <p>CAPPLA 1회, CAPPLM 1회, CDECIM 1회, CORGNI 1회, CUSERI 1회 — 총 5 쿼리로 처리합니다.
     */
    private void enrichCostListBatch(List<Bcostm> costs, List<CostDto.Response> responses) {
        if (costs.isEmpty()) return;

        // --- 1. CAPPLA 배치 조회 ---
        List<String> costBgNos =
                costs.stream().map(value -> value.getCostBgNo()).distinct().toList();
        List<com.kdb.it.common.approval.repository.ApplicationMapRepository.ApplicationMapView>
                allCapplas =
        ...
        for (com.kdb.it.common.approval.repository.ApplicationMapRepository.ApplicationMapView c :
                allCapplas) {
        ...
        List<String> apfMngNos =
                latestCappla.values().stream().map(value -> value.getApfDcmNo()).toList();
        Map<
                        String,
        ...
        // --- 3. CDECIM 배치 조회 ---
        List<com.kdb.it.common.approval.repository.ApproverRepository.ApproverReadView>
                allDecisions =
        ...
        Map<String, String> dfrCleCNameMap =
                dfrCleCCdvas.isEmpty()
                        ? Map.of()
                        : codeNameMapBuilder.build(CommonCodeGroups.DFR_CLE, dfrCleCCdvas);
        Map<String, String> tmnYnNameMap =
                tmnYnMngcCodes.isEmpty()
                        ? Map.of()
                        : codeNameMapBuilder.build(CommonCodeGroups.TMN_YN, tmnYnMngcCodes);
        Map<String, String> abusTcNameMap =
                abusTcCdvas.isEmpty()
                        ? Map.of()
                        : codeNameMapBuilder.build(CommonCodeGroups.ABUS, abusTcCdvas);
        Map<String, String> ioeCNameMap =
                ioeCCdvas.isEmpty() ? Map.of() : buildIoeCNameMap(ioeCCdvas);
        ...
        for (int i = 0; i < costs.size(); i++) {
            ...
            com.kdb.it.common.approval.repository.ApplicationMapRepository.ApplicationMapView
                    cappla = latestCappla.get(key);
            if (cappla != null) {
                response.setApfMngNo(cappla.getApfDcmNo());
                com.kdb.it.common.approval.repository.ApplicationRepository.ApplicationSummaryView
                        capplm = capplmMap.get(cappla.getApfDcmNo());
                if (capplm != null) {
                    response.setApfSts(
                            capplm.getItPtlApfPrgStsC() == null
                                    ? null
                                    : com.kdb.it.common.approval.domain.ApprovalStatus.ofCode(
                                            ...
                                            .label());
                    List<com.kdb.it.common.approval.repository.ApproverRepository.ApproverReadView>
                            decisions = decisionMap.getOrDefault(cappla.getApfDcmNo(), List.of());
                    response.setApplicationInfo(
                            ApplicationInfoDto.fromReadViews(capplm, decisions));
                }
            }
        ...
        for (Map.Entry<String, List<CostDto.Response>> entry : responsesByYear.entrySet()) {
            ...
            if (!prevAmtKeys.isEmpty()) {
                Map<String, BigDecimal> prevBgMap =
                        costRepository.sumPrevBgByCostBgNos(prevAmtKeys, prevYear);
                yearGroup.stream()
                        .filter(r -> "20".equals(r.getAbusTc()))
                        .forEach(
                                r ->
                                        r.setPrevBgAmt(
                                                prevBgMap.getOrDefault(
                                                        prevBudgetLookupKey(r), BigDecimal.ZERO)));
            }
            ...
            List<String> cncdNos =
                    yearGroup.stream()
                            ...
                            .toList();
            if (!cncdNos.isEmpty()) {
                Map<String, BigDecimal> prevDupBgMap =
                        bbugtmRepository.sumDupBgByItMngcNos(cncdNos, prevYear);
                yearGroup.forEach(
                        r -> {
                            if (r.getCncdRfrNo() != null && !r.getCncdRfrNo().isBlank()) {
                                r.setPrevDupBg(
                                        prevDupBgMap.getOrDefault(
                                                r.getCncdRfrNo(), BigDecimal.ZERO));
                            }
                        });
            }
        }
    ...
    /** 계속 항목의 전년도 예산 조회 키를 반환합니다. 전년도 항목이 cncdRfrNo로 연결된 경우 그 관리번호, 아니면 자기 관리번호(연차 데이터 호환). */
     ...
     *
     * <p>계속(abusTc='20') 항목만 대상이며, cncdRfrNo(전년도 관리번호) 우선 키로 전년도(bseYy-1) BCOSTM 예산금액 합계를 조회합니다. 목록
     * 배치 보강 ({@code enrichCostListBatch})과 동일한 기준입니다.
     */
    private void setPrevBudget(CostDto.Response response) {
        ...
        Map<String, BigDecimal> prevBgMap =
                costRepository.sumPrevBgByCostBgNos(List.of(key), prevYear);
    ...
    private void setCodeNames(CostDto.Response response) {
        ...
        if (response.getCostSvnDpmNm() == null
                && response.getCostSvnDpmC() != null
                && !response.getCostSvnDpmC().isEmpty()) {
            corgnIRepository
        ...
        if (response.getSvnTemNm() == null
                && response.getSvnTemC() != null
                && !response.getSvnTemC().isEmpty()) {
            corgnIRepository
        ...
        if (response.getTmnYn() != null && !response.getTmnYn().isEmpty()) {
            ...
            if (mngcTpCode != null) {
                ccodemRepository
                        .findByCIdAndCdvaWithValidDate(CommonCodeGroups.TMN_YN, mngcTpCode, null)
     ...
     *
     * <p>담당자명은 사번 배치 조회, 코드명은 그룹별 1회 조회로 N+1을 방지한다. 코드명은 cost-level(setCodeNames)과 동일한 CCODEM 유효일자
     * 기준 조회를 사용한다.
     */
    ...
    private Set<String> collectCdvas(
            List<CostDto.TerminalDto> dtos,
