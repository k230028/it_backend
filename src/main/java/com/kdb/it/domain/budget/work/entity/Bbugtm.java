...
public class Bbugtm extends BaseEntity {
     ...
     *
     * <p>물리컬럼 ASG_RT(메타표준=배정률)은 NUMBER(8,5)다. 종전에는 Integer로 매핑했으나, 하반기 계획 조정의 확정 편성금액 (예: 요청 1,406에
     * 대한 편성 416)은 29.58748%처럼 정수로 담기지 않는다. 물리 스케일을 그대로 쓰면 `요청금액 × 편성률 = 편성금액` 불변식이 확정금액 조정에서도 성립한다.
     */
