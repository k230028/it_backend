 ...
 *
 * <p>연도별 IT 부문 계획을 저장하는 마스터 엔티티입니다. 계획에 포함된 전체 프로젝트 데이터는 {@code redtConeInf}에 JSON 형태로 스냅샷 저장됩니다.
 *
 * <p>계획관리번호({@code reqDocNo}) 채번 규칙: {@code PLN-{대상년도}-{SQ_TPRMPP_BPLANM_1 시퀀스 4자리}} (예:
 * PLN-2026-0001)
 */
...
public class Bplanm extends BaseEntity {
     ...
     * 계획세부내용 (JSON 스냅샷)
     *
     * <p>계획 저장 시점의 전체 프로젝트 데이터를 JSON으로 직렬화하여 보관합니다. 계획 상세 화면에서 이 값을 파싱하여 표시합니다.
     */
    @Lob
    ...
    /** 5개 텍스트 필드를 갱신합니다. null 값은 해당 필드를 null로 초기화합니다. */
    ...
    @Column(
            ...
            precision = 18,
            scale = 3,
            comment = "총예산 (물리컬럼 ADU_TOT_AMT=합계총금액)")
    ...
    @Column(
            ...
            precision = 18,
            scale = 3,
            comment = "자본예산 (물리컬럼 TOT_CPIT_AMT=총자본금액)")
