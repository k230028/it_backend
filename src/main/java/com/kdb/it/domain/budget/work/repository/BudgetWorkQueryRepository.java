 ...
 *
 * <p>결재완료 원본 데이터를 단일 집계 쿼리로 조회하여 {@code BudgetWorkService.getSummary()} 내 N+1 쿼리를 제거합니다.
 */
public interface BudgetWorkQueryRepository {
     ...
     *
     * <p>BITEMM.amt는 저장 시점에 원화로 환산된 금액이므로 환율을 다시 곱하지 않고 합산합니다.
     *
     * @param bgYy 예산연도
     ...
     *
     * <p>결재완료 전산업무비(BCOSTM) {@code costBgNo}와 결재완료 정보화사업(BPROJM) 소속 품목(BITEMM) {@code gclMngNo}의
     * 합집합입니다. BBUGTM 편성액 집계 시 이 집합에 속한 원본(=BBUGTM.pkColNm)만 포함시켜, 미결재·취소된 원본의 stale 편성액이 합계를 부풀리는
     * 것을 방지하는 화이트리스트로 사용합니다.
     *
     * @param bgYy 예산연도
