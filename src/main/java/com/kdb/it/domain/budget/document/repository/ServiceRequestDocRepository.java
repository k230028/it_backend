 ...
 *
 * <p>Spring Data JPA의 {@link JpaRepository}를 상속하여 요구사항 정의서 테이블(TPRMPP_BRDOCM)의 기본 CRUD 기능을 제공합니다.
 *
 * <p>테이블 PK는 (DOC_MNG_NO, DOC_VRS) 복합키이며, 복합키 클래스 {@link BrdocmId} 를 사용합니다.
 *
...
public interface ServiceRequestDocRepository extends JpaRepository<Brdocm, BrdocmId> {
     ...
     */
    Optional<Brdocm> findByDocMngNoAndDocVrsSnoAndDelYn(
     ...
     *
     * <p>각 DOC_MNG_NO 그룹 내에서 가장 큰 DOC_VRS 행만 반환합니다. 삭제되지 않은(DEL_YN='N') 레코드만 대상으로 하며, 최초
     * 등록일시(FST_ENR_DTM) 내림차순으로 정렬합니다.
     *
     * @return 문서별 최신 버전 목록
     */
    @Query(
     ...
     *
     * <p>신규 요구사항 정의서 생성 시 문서관리번호 채번에 사용합니다. 형식: {@code DOC-{연도}-{4자리 시퀀스}} (예: {@code
     * DOC-2026-0001})
     *
    ...
    /** 부서 기준 최근 6개월 월별 문서 등록 건수 반환 컬럼: [0]=MONTH(YYYY-MM), [1]=CNT */
    @Query(
            value =
                    """
    ...
    /**
     * 부서 기준 검토 중인 최근 문서 3건 반환 컬럼: [0]=DOC_MNG_NO, [1]=REQ_NM, [2]=USR_NM,
     * [3]=CREATED_AT(YYYY-MM-DD), [4]=FSG_TLM(DATE)
     */
    @Query(
            value =
                    """
    ...
    default List<RecentReviewingRow> findRecentReviewingRowsByBbrC(String bbrC) {
        return findRecentReviewingByBbrC(bbrC).stream().map(RecentReviewingRow::fromRow).toList();
    }
}
