...
public class Bprojm extends BaseEntity {
     ...
     * 조회 응답에서 행번이 비워져 내려가므로 수정 저장이 빈값을 그대로 되돌려 보낸다. 그대로 덮으면 컬럼에 남아 있던 이름이 삭제되고 사용자 테이블 조인으로는 되살릴 수
     * 없다.
     *
     ...
     * 담당팀장명/담당자명 스냅샷 설정.
     *
     * <p><b>해석에 실패하면 기존 값을 유지합니다.</b> 담당자가 퇴사하면 {@code TPRMPP_CUSERI} 조인이 비는데, 그때 null로 덮으면 이 컬럼을 둔
     * 이유(퇴사자 이름 보존)가 사라집니다. 조인으로는 되살릴 수 없는 값이므로 한 번 채운 뒤에는 덮어쓰지 않는 쪽이 안전합니다.
     *
