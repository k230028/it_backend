...
public final class CostRepresentativeSelector {
     ...
     *
     * <p>선택 규칙: ① {@code LST_YN='Y'} 행 우선 → ② {@code BG_SNO} 내림차순(최신 일련번호). {@code LST_YN='Y'} 행이
     * 2건 이상이면 데이터 정합성 이상이므로 WARN 로그를 남기고 tie-break 결과를 사용합니다(장애 없이 동작).
     *
    ...
    private static <T> T pickByAccessors(
                ...
                costs.stream()
                        .min(
                                Comparator.comparing(
                                                (T cost) -> "Y".equals(lstYn.apply(cost)) ? 0 : 1)
                                        .thenComparing(
                                                bgSno,
        ...
        if (latestCount > 1) {
            log.warn(
                    "전산관리비 LST_YN='Y' 행이 {}건입니다 (costBgNo={}, 선택 bgSno={})",
                    latestCount,
                    costBgNo.apply(primary),
                    bgSno.apply(primary));
        }
        return primary;
    }
