 ...
 *
 * <p>각 업무 단계 서비스가 문서 생성/상태변경/삭제 시 호출하는 단일 진입점이다. {@code (프로젝트 ABUS_MNG_NO, 단계 자기 key, 통합
 * IT_PTL_STS_TC)}를 upsert 하거나 단계 문서 삭제 시 대응 BPROJA 행을 Soft Delete 한다. 멱등·방어적이며, 호출자 트랜잭션에 참여하므로 단계
 * 작업이 롤백되면 BPROJA 변경도 함께 롤백된다.
 */
@Service
...
public class BprojaSyncService {
    ...
    public void upsert(String abusMngNo, String cncdRfrNo, String itPtlStsTc) {
        if (!StringUtils.hasText(abusMngNo)
                || !StringUtils.hasText(cncdRfrNo)
                || itPtlStsTc == null) {
            return;
        }
        bprojaRepository
                .findById(new BprojaId(abusMngNo, cncdRfrNo))
                .ifPresentOrElse(
                        existing -> {
