...
public class DeliberationRepositoryImpl implements DeliberationRepositoryCustom {
     ...
     */
    @Override
    public List<DeliberationDto.ListItem> search(
        ...
        return queryFactory
                ...
                .from(d)
                .leftJoin(p)
                .on(p.abusMngNo.eq(d.cncdRfrNo).and(p.lstYn.eq("Y")).and(p.delYn.eq("N")))
                .leftJoin(c)
                .on(c.costBgNo.eq(d.cncdRfrNo).and(c.lstYn.eq("Y")).and(c.delYn.eq("N")))
                .where(where)
    ...
    public Optional<DeliberationDetailRow> findCurrentDetail(String docNo) {
        ...
        DeliberationDetailRow row =
                queryFactory
                        .select(
                                Projections.constructor(
                                        DeliberationDetailRow.class,
                                        d.docMngNo,
                                        d.docVrsSno,
                                        d.ioeC,
                                        d.cncdRfrNo,
    ...
    public Optional<DeliberationTargetRow> findCurrentWithTargetName(String docNo) {
        ...
        return Optional.of(
                new DeliberationTargetRow(row.get(0, Bdelim.class), row.get(1, String.class)));
    }
}
