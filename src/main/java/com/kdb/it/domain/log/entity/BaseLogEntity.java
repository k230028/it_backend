...
public abstract class BaseLogEntity {
     ...
     *
     * <p>DB 실제 컬럼은 {@code CHG_TC} (VARCHAR2(4), 모든 *L 테이블에서 NOT NULL).
     * Java 필드명은 의미 일관성을 위해 {@code chgTp}로 유지하며,
     * {@link com.kdb.it.domain.log.listener.AuditLogPersister#persist}가
     * {@code setField(logEntity, "chgTp", chgTp)}로 'C'/'U'/'D'를 채웁니다.</p>
     */
    @Column(name = "CHG_TC", length = 4, nullable = false, comment = "변경유형구분코드")
