 ...
 *
 * <p>실패 시 Micrometer 카운터({@code audit.log.write.failure})를 제한된 태그(entity/chgTp/stage)로 증가시키고 ERROR
 * 로그를 남긴다. 메트릭 기록 실패가 감사 실패 로깅을 막지 않도록 내부 try로 격리한다.
 *
 * <p>정적 ThreadLocal 플래그로 실패 처리 중 재진입을 차단해, 향후 catch 내부에서 감사 대상을 다시 건드려도 재귀·연쇄 실패가 발생하지 않게 한다.
 */
@Component
...
public class AuditFailureRecorder {
     ...
     *
     * <p>이미 실패 처리 중이면(재진입) 즉시 반환한다. 메트릭 기록 실패는 삼켜 로깅을 보장하고, 완료 후 ThreadLocal 상태를 제거한다.
     *
     ...
     */
    public void record(
            String entityName, String entityId, String chgTp, String stage, Exception cause) {
        ...
        try {
            ...
            log.error(
                    "[감사로그 기록 실패] entity={}, entityId={}, chgTp={}, stage={}",
                    entityName,
                    entityId,
                    chgTp,
                    stage,
