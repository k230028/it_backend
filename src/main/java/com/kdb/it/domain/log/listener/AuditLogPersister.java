 ...
 *
 * <p>원본 엔티티의 {@code @Column} 필드를 리플렉션으로 복사해 로그 엔티티 스냅샷을 만들고, 원 업무 트랜잭션 커밋 이후({@code afterCommit})
 * {@link AuditLogWriter}가 별도 트랜잭션에서 INSERT하도록 예약한다. 활성 동기화가 없으면 즉시 direct 단계로 기록한다.
 *
 * <p>스냅샷 생성은 {@code @PrePersist}/{@code @PreUpdate} 시점에 동기로 수행되어 원본과 동일한 GUID·기본값을 확보하고, DB 쓰기만 커밋
 * 이후로 분리한다. 쓰기 실패는 {@link AuditFailureRecorder}가 카운터·로그로 기록한다.
 */
@Component
...
public class AuditLogPersister {
     ...
     */
    public void persist(
            Object sourceEntity, Class<? extends BaseLogEntity> logClass, String chgTp) {
     ...
     *
     * <p>로그 엔티티를 인스턴스화하고 공통 감사값·변경 메타·{@code @Column} 값을 복사합니다. 스냅샷 생성 자체가 실패하면 예외를 전파해 상위 리스너가 실패
     * recorder로 위임합니다.
     *
     ...
     */
    private BaseLogEntity createSnapshot(
            Object sourceEntity, Class<? extends BaseLogEntity> logClass, String chgTp) {
        try {
     ...
     *
     * <p>{@link com.kdb.it.domain.entity.BaseEntity#prePersist()}와 동일한 규칙이며, 값이 null인 경우에만 채우므로
     * 멱등합니다. 생성(C) 로그 스냅샷이 NOT NULL 제약을 만족하고 원본 행과 동일한 GUID를 갖도록, prePersist보다 먼저(스냅샷 직전) 적용합니다. 해당
     * 필드가 없는 엔티티(비-BaseEntity 로그 대상)는 조용히 건너뜁니다.
     *
     ...
     *
     * <p>상속 계층 전체({@link #collectColumnFields})를 탐색하므로 {@link com.kdb.it.domain.entity.BaseEntity}의
     * 공통 컬럼도 복사됩니다.
     *
     * @param source 원본 엔티티 (CUD 이벤트 발생 엔티티)
     ...
     *
     * <p>{@code name}이 빈 문자열이면 JPA 기본값 규칙(필드명 그대로 사용)을 따릅니다.
     *
     ...
     *
     * <p>{@link Object}까지 순회하므로 엔티티 상속 구조에서도 공통 컬럼이 누락되지 않습니다.
     *
     ...
     *
     * <p>클래스 계층 전체를 탐색하여 필드를 찾습니다. 주로 {@link BaseLogEntity}의 {@code chgTp}, {@code chgDtm}, {@code
     * chgUsid} 설정에 사용됩니다.
     *
     ...
     *
     * <p>인증 컨텍스트가 없거나 미인증 상태이면 {@code null}을 반환합니다. 배치 또는 비인증 컨텍스트에서 호출되는 경우에도 안전하게 처리됩니다.
     *
