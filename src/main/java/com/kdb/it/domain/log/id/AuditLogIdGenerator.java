 ...
 *
 * <p>로그 엔티티의 {@code @Table(name)} 값(테이블명)으로부터 Oracle 시퀀스 {@code SQ_{테이블명}_1.NEXTVAL}을 조회하여 {@code
 * Long} 값을 반환한다.
 *
 * <p>예: {@code TPRMPP_BPROJL} → {@code SQ_TPRMPP_BPROJL_1.NEXTVAL} (숫자)
 */
public class AuditLogIdGenerator implements IdentifierGenerator {
     ...
     *
     * <p>채번 흐름: {@code @Table(name)} 추출 → {@code SQ_{테이블명}_1.NEXTVAL} 조회
     *
     * <p>예: {@code TPRMPP_BPROJL} → {@code SQ_TPRMPP_BPROJL_1.NEXTVAL} → Long 값 반환
     *
     * <p>시퀀스 미존재(ORA-02289) 등 DB 오류 발생 시 {@link RuntimeException}으로 래핑하여 전파합니다. 해당 예외는 {@link
     * com.kdb.it.domain.log.listener.ChangeLogEntityListener}가 삼켜 원본 트랜잭션 롤백을 방지합니다.
     *
