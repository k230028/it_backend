 ...
 * JPA 엔티티 변경 이벤트 리스너.
 *
 * <p>{@link LogTarget}이 붙은 엔티티의 {@code @PrePersist} / {@code @PreUpdate} 이벤트를 감지하여 {@link
 * AuditLogPersister}로 로그 INSERT를 위임한다.
 *
 * <p>JPA가 직접 인스턴스화하므로 Spring 빈이 아니며, {@link ApplicationContextHolder}를 통해 {@link
 * AuditLogPersister}를 조회한다.
 *
 * <p>{@code @PostPersist}/{@code @PostUpdate} 대신 {@code @PrePersist}/{@code @PreUpdate}를 사용하는 이유:
 * Post 콜백은 Hibernate ActionQueue 이터레이션 도중 호출되므로 {@code entityManager.persist()}를 호출하면 {@link
 * java.util.ConcurrentModificationException}이 발생한다. Pre 콜백은 이터레이션 이전에 호출되므로 안전하다.
 *
 * <p>동일 트랜잭션 내 AUTO flush → commit flush 이중 실행 방지: {@code AuditingEntityListener}가
 * {@code @PreUpdate}에서 {@code lstChgDtm}을 변경하면 Hibernate가 dirty 재감지하여 {@code @PreUpdate}가 두 번 호출될 수
 * 있다. {@code inFlightEntities}(identity 기반 ThreadLocal Set)로 flush 사이클당 1회만 기록한다.
 */
public class ChangeLogEntityListener {

    private static final org.slf4j.Logger log =
            LoggerFactory.getLogger(ChangeLogEntityListener.class);
     ...
     *
     * <p>{@link LogTarget} 어노테이션이 없는 엔티티는 처리하지 않습니다.
     *
     ...
     * JPA UPDATE 전 처리 진입점.
     *
     * <p>동일 flush 사이클 내 동일 인스턴스에 대한 중복 로그 방지 로직이 포함되어 있습니다. {@link LogTarget} 어노테이션이 없는 엔티티는 처리하지
     * 않습니다.
     *
     ...
     * JPA UPDATE 후 처리 진입점.
     *
     * <p>flush 사이클 완료 후 {@code inFlightEntities}에서 해당 인스턴스를 제거하여 다음 flush 사이클에서 재기록이 가능하도록 정리합니다.
     *
     ...
     *
     * <p>{@link AuditLogPersister}를 통해 로그 엔티티를 생성하고 저장합니다. 로그 저장 실패 시 예외를 삼켜 원본 트랜잭션 롤백을 방지합니다.
     *
     ...
     *
     * <p>리플렉션으로 {@code delYn} 필드를 조회합니다. 필드가 없으면 수정(U)으로 간주하며, 접근 실패 시에도 수정(U)을 반환합니다.
     *
    ...
    private String resolveUpdateType(Object entity) {
        ...
        try {
            ...
            log.warn(
                    "[감사로그] delYn 리플렉션 실패 — 변경유형 U로 폴백: entity={}",
                    entity.getClass().getSimpleName(),
                    e);
            return "U";
        }
     ...
     *
     * <p>현재 클래스부터 상위 클래스를 순서대로 탐색합니다. 필드를 찾지 못하면 {@code null}을 반환합니다 (예외 미발생).
     *
