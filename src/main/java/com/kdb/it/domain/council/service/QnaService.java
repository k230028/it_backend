 ...
 *
 * <p>협의회 개최 전 평가위원이 사전 질의를 등록하고, 추진부서 담당자(ITPZZ001)가 답변합니다.
 *
 * <p>QTN_ID 형식: {@code QTN-{asctId}-{2자리순번}} (예: QTN-ASCT-2026-0001-01)
 *
 * <p>설계 참조: §2.5 API 설계 — 질의응답 엔드포인트
 */
@Service
...
public class QnaService {
    ...
    public List<CouncilDto.QnaResponse> getQnaList(String asctId) {
        ...
        return qnaRepository.findByItPtlAsctIdAndDelYnOrderByFstEnrDtmAsc(asctId, "N").stream()
                .map(this::toResponse)
                .toList();
    }
     ...
     * 사전 질의 등록 (평가위원)
     *
     * <p>QTN_ID를 자동 채번하여 새 질의를 저장합니다. REP_YN='N' (미답변) 상태로 등록됩니다.
     *
     * @param asctId 협의회ID
     ...
     * 사전 질의 수정 (질의 등록자)
     *
     * <p>질의 내용(QTN_CONE)을 업데이트합니다. 본인이 등록한 질의만 수정할 수 있습니다.
     *
     * @param asctId 협의회ID
     * @param qtnId 질의응답ID
