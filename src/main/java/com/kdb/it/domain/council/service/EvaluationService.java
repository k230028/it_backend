 ...
 *
 * <p>평가위원이 협의회 당일 또는 이후에 6개 점검항목에 대해 점수와 의견을 작성합니다.
 *
 * <p>평가의견 작성 규칙:
 *
 * <ul>
 *   <li>6개 항목(MGMT_STR/FIN_EFC/RISK_IMP/REP_IMP/DUP_SYS/ETC) 전체 입력 필수
 *   <li>점수 1~2점 입력 시 의견(ckgOpnn) 작성 필수
 *   <li>기존 의견이 있으면 update, 없으면 신규 INSERT (upsert)
 *   <li>첫 제출 시 협의회 상태를 EVALUATING으로 전이
 * </ul>
 *
 * <p>설계 참조: §2.1 EvaluationService — 3단계 담당
 */
@Service
...
public class EvaluationService {
    ...
    private static final Map<String, String> CHECK_ITEM_NAMES =
            Map.of(
                    "01", "경영전략/계획 부합",
                    "02", "재무 효과",
                    "03", "리스크 개선 효과",
     ...
     *
     * <p>전체 위원별 평가의견 목록 + 점검항목별 평균점수를 반환합니다. 결과서 작성 화면에서 참고 데이터로 활용됩니다.
     *
     * @param asctId 협의회ID
    ...
    public CouncilDto.EvaluationSummaryResponse getAllEvaluations(String asctId) {
        ...
        Map<String, UserRepository.UserNameView> userMap =
                buildUserMapFromEvaluations(allEvaluations);

        // 평가의견 → 응답 DTO 변환
        List<CouncilDto.EvaluationItemResponse> evaluationResponses =
                        ...
                        .map(
                                e -> {
                                    ...
                                    return new CouncilDto.EvaluationItemResponse(
                                            ...
                                            e.getItPtlCkgItmTc(),
                                            CHECK_ITEM_NAMES.getOrDefault(
                                                    e.getItPtlCkgItmTc(), e.getItPtlCkgItmTc()),
                                            e.getQuelRcrd(),
                                            e.getCkgOpnn());
                                })
                        .toList();
    ...
    public List<CouncilDto.EvaluationItemResponse> getMyEvaluation(
        ...
        return myEvaluations.stream()
                .map(
                        e ->
                                new CouncilDto.EvaluationItemResponse(
                                        e.getEno(),
                                        null, // 본인 조회 시 성명 불필요
                                        e.getItPtlCkgItmTc(),
                                        CHECK_ITEM_NAMES.getOrDefault(
     ...
     * 평가의견 작성/수정 (평가위원)
     *
     * <p>6개 점검항목을 upsert합니다. 1~2점 입력 시 의견(ckgOpnn) 작성이 필수입니다. 첫 제출 시 협의회 상태를 IN_PROGRESS →
     * EVALUATING으로 전이합니다.
     *
     * <p>Plan SC: 상태 전이는 EVALUATING이 아닌 경우에만 수행 (중복 전이 방지)
     *
     * @param asctId 협의회ID
    ...
    public void saveEvaluation(
            String asctId, CouncilDto.EvaluationRequest request, CustomUserDetails userDetails) {
        ...
        Map<String, Bevalm> existingByItem =
                evaluationRepository.findByItPtlAsctIdAndEnoAndDelYn(asctId, eno, "N").stream()
                        .collect(
                                Collectors.toMap(
                                        evaluation -> evaluation.getItPtlCkgItmTc(),
                                        e -> e,
                                        (a, b) -> a));
        ...
        for (CouncilDto.EvaluationItem item : request.items()) {
            ...
            if (item.ckgRcrd() != null && item.ckgRcrd() <= 2) {
                if (item.ckgOpnn() == null || item.ckgOpnn().isBlank()) {
                    ...
                    throw new IllegalArgumentException("점수 1~2점 입력 시 의견 작성이 필수입니다. 항목: " + itemNm);
                }
            }
     ...
     *
     * <p>위원 전원의 사번을 조회한 후, 각 사번에 대해 6개 항목 제출 여부를 검증합니다. 단 한 명이라도 미제출 항목이 있으면 false를 반환합니다.
     *
     * @param asctId 협의회ID
     * @return 전원 제출 완료 여부
     */
                ...
                allEvaluations.stream()
                        .collect(
                                Collectors.groupingBy(
                                        value -> value.getEno(), Collectors.counting()));
     ...
     *
     * <p>사번 집합을 모아 사용자 이름 프로젝션으로 일괄 조회합니다.
     */
    private Map<String, UserRepository.UserNameView> buildUserMapFromEvaluations(
            List<Bevalm> evaluations) {
        List<String> enos =
                evaluations.stream().map(evaluation -> evaluation.getEno()).distinct().toList();
        if (enos.isEmpty()) {
            return Map.of();
        }
     ...
     * 점검항목별 평균점수 목록 생성
     *
     * <p>native query 결과(Object[])를 CheckItemAvgScore DTO로 변환하고 고정 항목 순서(CHECK_ITEM_ORDER)에 맞게
     * 정렬합니다.
     *
     * @param asctId 협의회ID
    ...
    public List<CouncilDto.CheckItemAvgScore> buildAvgScores(String asctId) {
        ...
        return CHECK_ITEM_ORDER.stream()
                ...
                .map(
                        code ->
                                new CouncilDto.CheckItemAvgScore(
                                        code,
