 ...
 *
 * <p>타당성검토표 작성완료(SUBMITTED) 후 팀장 결재를 공통 전자결재 시스템과 연동합니다.
 *
 * <p>결재 흐름:
 *
 * <pre>
...
public class CouncilApprovalService {
     ...
     *
     * <p>전자결재 시스템에 신청서를 등록하고 협의회 상태를 APPROVAL_PENDING으로 전이합니다.
     *
     * <p>Plan SC: 결재자는 소관부서 팀장 1인 (approverEno)으로 단일 결재선 구성
     *
     * @param asctId 협의회ID
    ...
    public CouncilDto.ApprovalResponse requestApproval(
            String asctId, CouncilDto.ApprovalRequest request, CustomUserDetails userDetails) {
        ...
        ApplicationDto.CreateRequest createRequest =
                buildApprovalRequest(
                        apfNm,
                        asctId,
                        List.of(request.approverEno()),
                        request.rqsOpnn(),
                        userDetails.getEno());
        ...
        councilService.changeStatus(asctId, "03");
     ...
     *
     * <p>전원 결과서 확인 완료 후 IT관리자가 부장에게 결재를 요청합니다. 전자결재 시스템에 신청서를 등록하고 협의회 상태를
    ...
    public CouncilDto.ApprovalResponse requestResultApproval(
        ...
        if (!"11".equals(council.getItPtlAsctPrgStsTc())) {
            throw new IllegalStateException(
                    "개최결과서 결재 요청은 결재 요청 가능(011) 상태에서만 가능합니다. 현재 상태: "
        ...
        ApplicationDto.CreateRequest createRequest =
                buildApprovalRequest(
                        apfNm,
                        asctId,
                        List.of(request.teamLeadEno(), request.deptHeadEno()),
                        request.rqsOpnn(),
                        userDetails.getEno());

        String apfMngNo = applicationService.submit(createRequest);

        // 협의회 상태 전이: FINAL_APPROVAL → RESULT_APPROVAL_PENDING (PRD §31: 결재 중 = 012)
        councilService.changeStatus(asctId, "12");
     ...
     *
     * <p>결재 시스템에서 승인 또는 반려 처리 시 이 메서드로 협의회 상태를 업데이트합니다. 현재 상태에 따라 두 가지 결재 흐름을 분기 처리합니다:
     *
     * <ul>
     *   <li>APPROVAL_PENDING (타당성검토표 결재): 승인 → APPROVED, 반려 → DRAFT
     *   <li>RESULT_APPROVAL_PENDING (개최결과서 결재): 승인 → COMPLETED, 반려 → FINAL_APPROVAL (재결재 가능)
     * </ul>
     *
     * @param asctId 협의회ID
    ...
    public void processApprovalCallback(String asctId, CouncilDto.ApprovalCallbackRequest request) {
        ...
        if ("03".equals(currentStatus)) {
            ...
            if (request.approved()) {
                // 승인: 003 → 004
                councilService.changeStatus(asctId, "04");
            } else {
                // 반려: 003 → 001 (타당성검토표 재작성)
                councilService.changeStatus(asctId, "01");
            }
        } else if ("12".equals(currentStatus)) {
            // 개최결과서 결재 콜백 (RESULT_APPROVAL_PENDING, PRD §31: 결재 중 = 012)
            if (request.approved()) {
                // 승인: 012(결재 중) → 013(완료)
                councilService.changeStatus(asctId, "13");
            } else {
                // 반려: 012 → 011 (재결재 요청 가능)
                councilService.changeStatus(asctId, "11");
            }
        } else {
            throw new IllegalStateException("결재 콜백 처리는 결재대기 상태에서만 가능합니다. 현재 상태: " + currentStatus);
        }
    }
