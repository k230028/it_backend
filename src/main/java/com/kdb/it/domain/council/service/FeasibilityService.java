 ...
 * 타당성검토표 서비스 (Step 1)
 *
 * <p>타당성검토표는 3개 엔티티로 구성됩니다:
 *
 * <ul>
 *   <li>{@code BPOVWM}: 사업개요 (1:1)
 *   <li>{@code BPERFM}: 성과관리 자체계획 (1:N, 동적 추가/삭제)
 * </ul>
 *
 * <p>저장 방식:
 *
 * <ul>
 *   <li>KPN_TP_TC=10(임시저장): 상태 DRAFT 유지, 빈 값 허용
 *   <li>KPN_TP_TC=20(저장완료): 상태 SUBMITTED 전이, 첨부파일 필수
 * </ul>
 *
 * <p>성과지표 저장 전략: 요청에 포함된 전체 목록으로 교체 (기존 삭제 + 신규 저장)
 *
 * <p>설계 참조: §2.1 FeasibilityService — 1단계 담당
 */
@Service
...
public class FeasibilityService {
     ...
     *
     * <p>신규 작성이면 INSERT, 기존 데이터가 있으면 UPDATE합니다.
     *
     * <p>작성완료(KPN_TP_TC=20) 시 처리:
     *
     * <ul>
     *   <li>첨부파일(hwp/hwpx/pdf) 필수 확인
     *   <li>협의회 상태를 SUBMITTED로 전이
     * </ul>
     *
     * @param asctId 협의회ID
     ...
     *
     * <p>협의회 단위로 점검항목당 1행(BCHKLM)을 유지한다. 평가위원 평가(BEVALM)와 달리 사번이 없다. 작성완료(kpnTc='20') 시 모든 항목의
     * 점검점수와 점검의견 입력이 필수이며, 임시저장('10')은 부분 입력을 허용한다.
     */
    private void saveOrUpdateSelfChecks(String asctId, CouncilDto.FeasibilityRequest req) {
        ...
        Map<String, Bchklm> existingByItem =
                selfCheckRepository.findByItPtlAsctIdAndDelYn(asctId, "N").stream()
     ...
     *
     * <p>동적 추가/삭제를 지원하기 위해 요청 목록으로 완전 교체합니다.
     *
     * <p>소프트 딜리트(del_yn='Y') 방식을 사용하지 않는 이유: BPERFM은 복합 PK(itPtlAsctId, evlDtpSno)를 가지며 DEL_YN이 NOT
     * NULL입니다. soft-delete 후 동일 PK로 재삽입 시 JPA merge()가 del_yn=null로 덮어써 NOT NULL 제약 위반이 발생합니다. 하드
     * 딜리트로 PK 충돌을 방지합니다.
     */
    private void replacePerformances(String asctId, List<CouncilDto.PerformanceRequest> requests) {
        ...
        entityManager
                .createQuery("DELETE FROM Bperfm b WHERE b.itPtlAsctId = :asctId")
     ...
     *
     * <p>Plan SC: 첨부파일은 hwp/hwpx/pdf만 허용 (보안 정책)
     *
    ...
    private CouncilDto.FeasibilityResponse toFeasibilityResponse(
        ...
        // 자체점검 변환
        List<CouncilDto.SelfCheckItemResponse> selfCheckResponses =
                selfChecks.stream()
                        .map(
                                c ->
                                        new CouncilDto.SelfCheckItemResponse(
                                                c.getItPtlCkgItmTc(),
                                                c.getQuelRcrd(),
                                                c.getCkgOpnn()))
                        .toList();
        ...
        // 성과지표 변환
        List<CouncilDto.PerformanceResponse> perfResponses =
                performances.stream()
                        .map(
                                p ->
                                        new CouncilDto.PerformanceResponse(
                                                p.getEvlDtpSno(),
                                                p.getEvlDtpNm(),
                                                p.getEvlDtpDfntCone(),
                                                p.getEvlDtpClfCone(),
        ...
        return new CouncilDto.FeasibilityResponse(
                overview.getAbusNm(),
                overview.getAbusTrmCone(),
                overview.getAbusNcsCone(),
                overview.getRqmBgAmt(),
                overview.getItPtlEdrtTc(),
                overview.getAbusCone(),
                overview.getLwRglYn(),
                overview.getLwFdtn(),
                overview.getDgogPpoCone(),
                overview.getKpnTpTc(),
                perfResponses,
                overview.getFlMpnId(),
                selfCheckResponses);
