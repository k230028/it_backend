 ...
 *
 * <p>평가위원이 가능한 일정을 입력하면 IT관리자가 최종 일정을 확정합니다.
 *
 * <p>일정 확정 흐름:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>일정 허용 시간대: 10:00 / 14:00 / 15:00 / 16:00
 *
 * <p>설계 참조: §2.1 ScheduleService — 2단계 담당
 */
@Service
...
public class ScheduleService {
     ...
     * null로 덮어쓰는 회귀가 있어 직접 persist를 사용합니다(PRD §15/§17).
     */
     ...
     * 일정 입력 현황 조회 (IT관리자용)
     *
     * <p>전체 위원 목록과 각 위원의 일정 응답 현황을 반환합니다. 미응답 위원이 없을 때 일정확정 버튼이 활성화됩니다.
     *
     * @param asctId 협의회ID
    ...
    public CouncilDto.ScheduleStatusResponse getScheduleStatus(String asctId) {
        ...
        Set<String> respondedEnos =
                allSchedules.stream().map(value -> value.getEno()).collect(Collectors.toSet());

        // 위원별 응답용 사용자 정보 Map
        Map<String, UserRepository.CouncilMemberUserRow> userMap = buildUserMap(members);

        // 위원별 일정 응답 목록 Map (eno → slots)
        Map<String, List<Bschdm>> scheduleByEno =
                allSchedules.stream().collect(Collectors.groupingBy(value -> value.getEno()));

        // 위원별 현황 생성
        List<CouncilDto.MemberScheduleStatus> memberStatuses =
                members.stream()
                        .map(
                                m -> {
                                    boolean responded = respondedEnos.contains(m.getEno());
                                    UserRepository.CouncilMemberUserRow user =
                                            userMap.get(m.getEno());

                                    List<CouncilDto.ScheduleSlotResponse> slots =
                                            scheduleByEno
                                                    .getOrDefault(m.getEno(), List.of())
                                                    .stream()
                                                    .map(
                                                            s ->
                                                                    new CouncilDto
        ...
        boolean anyFaceToFaceHope = members.stream().anyMatch(m -> "Y".equals(m.getCsfHpYn()));

        // 응답/미응답 위원 수 — 간사 제외된 취합 대상(members) 기준으로 계산
        int respondedCount =
                (int) members.stream().filter(m -> respondedEnos.contains(m.getEno())).count();
     ...
     * 내 일정 응답 조회 (평가위원 본인)
     *
     * <p>로그인한 평가위원이 이미 제출한 일정 슬롯 목록을 반환합니다. 제출 이력이 없으면 빈 목록을 반환합니다.
     *
     * @param asctId 협의회ID
     * @param eno 로그인한 사번
    ...
    public List<CouncilDto.ScheduleSlotResponse> getMySchedule(String asctId, String eno) {
        ...
        return scheduleRepository.findByItPtlAsctIdAndEnoAndDelYn(asctId, eno, "N").stream()
                .map(
                        s ->
                                new CouncilDto.ScheduleSlotResponse(
     ...
     *
     * <p>심의유형과 무관하게 평가위원 '전원'이 가능 일정을 제출해야 일정 확정이 가능합니다. (이전에는 INFO_SYS의 경우 필수 팀장(예산팀장 12004 ·
     * IT기획팀장 18001)만 응답하면 확정 가능했으나, 전원 응답 기준으로 통일했습니다.)
     *
     * @param members 전체 위원 목록
     * @param respondedEnos 응답 완료한 위원 사번 Set
     * @return 위원 전원이 응답했으면 true
     */
    private boolean calcAllRequiredResponded(List<Bcmmtm> members, Set<String> respondedEnos) {
     ...
     * 일정 입력 (평가위원)
     *
     * <p>평가위원이 날짜×시간대별 가능 여부를 입력합니다. 기존 응답이 있으면 update(respond), 없으면 신규 INSERT합니다.
     *
     * <p>Plan SC: 허용 시간대(10:00/14:00/15:00/16:00)만 저장
     *
     * @param asctId 협의회ID
    ...
    public void submitSchedule(
            String asctId, CouncilDto.ScheduleRequest request, CustomUserDetails userDetails) {
        councilService.findActiveCouncil(asctId);
        ...
        if (request.csfHopeYn() != null) {
            committeeRepository
                    .findByItPtlAsctIdAndEnoAndDelYn(asctId, eno, "N")
     ...
     *
     * <p>이미 8자 yyyyMMdd 형식이거나 null이면 그대로 반환합니다.
     */
     ...
     * 일정 확정 (IT관리자)
     *
     * <p>최종 회의 일정을 BASCTM에 반영하고 협의회 상태를 SCHEDULED로 전이합니다.
     *
     * <p>Plan SC: 확정 후 SCHEDULED 상태 전이, BASCTM.CNRC_DT/TM/PLC 업데이트
     *
     * @param asctId 협의회ID
     ...
     *
     * <p>위원 전원이 대면을 희망하지 않을 때, IT관리자가 서면개최로 확정합니다. 회의일자/시간/장소 없이 BASCTM.CSF_HELD_YN='N'으로 설정하고,
     * 일정확정(SCHEDULED)·개최(개최시작) 단계를 건너뛰어 상태를 바로 진행중(07)으로 전이합니다. 이후 서면질의응답 → 평가의견 작성 흐름은 대면과 동일합니다.
     *
     * @param asctId 협의회ID
