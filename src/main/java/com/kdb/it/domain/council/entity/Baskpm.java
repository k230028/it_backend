 ...
 *
 * <p>DB 테이블: {@code TPRMPP_BASKPM} (협의회당 1건, PK=협의회ID)
 *
 * <p>정보보호기획(ITPAD002)이 IT기획(ITPAD001) 앞으로 보내는 타당성검토 생략 판정 요청을 적재합니다. IT기획이 생략여부를 판정한 뒤
 * 전자결재(IT기획팀장→부장)를 거쳐 확정합니다.
 *
 * <p>진행상태는 별도 컬럼 없이 확인일시(CNFM_DTM)/생략여부(PRTY_IVG_OMT_YN)/결재·협의회 상태로 파생합니다.
 */
...
public class Baskpm extends BaseEntity {
     ...
     *
     * <p>NOT NULL 컬럼은 감사로그 스냅샷 타이밍을 위해 생성 시점에 모두 채웁니다(it_backend §5.12.1.1).
     *
     ...
     *
     * <p>최종 생략여부·사유는 협의회 마스터(BASCTM)에 기록하고, 이 테이블에는 판정 접수 메타데이터만 남깁니다. 회신 전(요청) 상태는 확인일시(CNFM_DTM)
     * null 여부로 판별하므로 별도 처리상태 컬럼을 두지 않습니다.
     *
