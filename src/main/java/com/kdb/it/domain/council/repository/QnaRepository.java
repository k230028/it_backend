...
public interface QnaRepository extends JpaRepository<Bpqnam, BpqnamId> {
     ...
     * @return 미답변 질의 목록
     */
    List<Bpqnam> findByItPtlAsctIdAndQtnRpdRltYnAndDelYn(
            String itPtlAsctId, String qtnRpdRltYn, String delYn);

    /**
     * QTN_ID 채번 (협의회ID 기반 순번)
     *
     * <p>새로운 질의 등록 시 ID를 생성합니다. 형식: {@code QTN-{itPtlAsctId}-{2자리순번}} (예: QTN-ASCT-2026-0001-01)
     *
     * @param itPtlAsctId 협의회ID
    ...
    @Query(
            value =
                    "SELECT NVL(COUNT(*), 0) + 1 FROM TPRMPP_BPQNAM WHERE IT_PTL_ASCT_ID = :itPtlAsctId",
            nativeQuery = true)
