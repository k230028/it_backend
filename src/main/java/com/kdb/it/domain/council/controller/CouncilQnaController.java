 ...
 *
 * <p>기본 URL은 {@code /api/council}이며, 협의회 개최 전 질의응답 엔드포인트만 담당합니다.
 */
@RestController
...
public class CouncilQnaController {
     ...
     * 사전질의응답 목록 조회.
     *
     * <p>해당 협의회의 전체 질의응답 목록을 등록일시 오름차순으로 반환합니다. 평가위원과 추진부서 담당자 모두 조회 가능합니다.
     *
     * @param asctId 협의회ID
     ...
     * 사전 질의 등록 (평가위원).
     *
     * <p>평가위원이 협의회 개최 전 사전 질의를 등록합니다. QTN_ID는 자동 채번됩니다.
     *
     * @param asctId 협의회ID
    ...
    public ResponseEntity<String> createQna(
        ...
        return ResponseEntity.created(
                        java.net.URI.create("/api/council/" + asctId + "/qna/" + qtnId))
                .body(qtnId);
    }
     ...
     * 사전 질의 답변 (추진부서 담당자).
     *
     * <p>추진부서 담당자(ITPZZ001)가 평가위원의 사전 질의에 답변합니다. 답변 후 REP_YN='Y'로 변경됩니다.
     *
     * @param asctId 협의회ID
     * @param qtnId 질의응답ID
     * @param request 답변 요청
