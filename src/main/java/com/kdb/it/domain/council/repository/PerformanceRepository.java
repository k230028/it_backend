 ...
 *
 * <p>협의회 1건당 1개 이상의 성과지표를 동적으로 관리합니다.
 */
public interface PerformanceRepository extends JpaRepository<Bperfm, BperfmId> {
     ...
     * 협의회의 다음 성과지표 순번 계산
     *
     * <p>성과지표 추가 시 자동으로 순번을 채번합니다. 삭제된 항목은 제외하고 현재 최대 순번 + 1을 반환합니다.
     *
     * @param itPtlAsctId 협의회ID
    ...
    @Query(
            value =
                    "SELECT NVL(MAX(EVL_DTP_SNO), 0) + 1 FROM TPRMPP_BPERFM WHERE IT_PTL_ASCT_ID = :itPtlAsctId AND DEL_YN = 'N'",
            nativeQuery = true)
