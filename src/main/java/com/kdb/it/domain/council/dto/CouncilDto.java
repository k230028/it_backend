...
public class CouncilDto {
     ...
     *
     * <p>SUBMITTED 상태인 협의회의 타당성검토표를 팀장에게 결재 요청합니다. 신청자 사번은 JWT에서, 신청의견은 선택사항입니다.
     *
     ...
     *
     * <p>팀장 결재 완료/반려 시 협의회 상태를 업데이트하는 콜백 요청입니다.
     *
     ...
     *
     * <p>FINAL_APPROVAL 상태에서 IT관리자가 결재자(부장)를 지정하여 전자결재 시스템에 개최결과서 결재를 신청합니다.
     *
     ...
     * 사전 질의 등록 요청 (평가위원)
     *
     * <p>평가위원이 협의회 개최 전 사전 질의를 등록합니다. 질의자 사번은 JWT에서 자동 주입됩니다.
     *
     * @param qtnCone 질의내용
     */
     ...
     *
     * <p>추진부서 담당자(ITPZZ001)가 질의에 답변합니다. 답변자 사번은 JWT에서 자동 주입됩니다.
     *
     * @param repCone 답변내용
     */
     ...
     * 추진부서 통보 응답
     *
     * <p>통보 완료 후 수신자(추진부서 담당자) 정보를 반환합니다.
     *
     * @param eno 수신자 사번
     * @param usrNm 수신자 성명
     * @param bbrNm 수신자 부서명
     ...
     *
     * <p>타당성검토표 첨부는 Step1(Bpovwm)의 기존 첨부를 서버가 재사용하므로 클라이언트가 보내지 않습니다. 사업계획서 첨부만 신규 업로드 후 그 파일관리번호를
     * 전달합니다.
     *
     ...
     *
     * <p>{@code decided}=확인일시(cnfmDtm) 존재 여부. 회신 전이면 false(판정 대기).
     *
     * @param asctId 협의회ID
     * @param rsn 담당자의견내용 (요청 설명)
