 ...
 *
 * <p>{@link ApprovalCompletedEvent}를 구독하여, 신청서가 BASKPM(생략판정요청) 원본에 연결된 경우 {@link
 * CouncilSkipService#handleApprovalCompleted}로 분기 처리(생략→skip / 개최→prepare / 반려→통보)합니다.
 *
 * <p>{@code @EventListener} + {@code @Transactional} 조합으로 발행자(ApplicationService.approve)와 동일
 * 트랜잭션에서 동기 처리합니다(협의회 결재 리스너와 동일 패턴).
 */
@Component
...
public class CouncilSkipApprovalEventListener {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(CouncilSkipApprovalEventListener.class);
    ...
    public void handleApprovalCompleted(ApprovalCompletedEvent event) {
        List<Cappla> links =
                applicationMapRepository.findByApfDcmNoAndFntTbNm(
                        event.apfMngNo(), CouncilSkipService.ORC_TB_CD);
        if (links.isEmpty()) {
