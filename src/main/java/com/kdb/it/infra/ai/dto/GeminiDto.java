 ...
 *
 * <p>프론트엔드 ↔ 백엔드 ↔ Gemini API 간 데이터 전달에 사용합니다.
 */
public class GeminiDto {
    ...
    public static class Request {
        ...
        /** 시스템 지시문 (선택) Gemini에게 역할이나 응답 방식을 사전에 지정할 때 사용합니다. */
         ...
         * 첨부파일 관리번호 목록 (선택)
         *
         * <p>TPRMPP_CFILEM에 저장된 파일의 관리번호를 전달하면 해당 파일을 Base64로 인코딩하여 Gemini에 함께 전송합니다.
         *
    ...
    public static class Response {
        ...
        /** 실제로 Gemini에 첨부된 파일 수 0이면 첨부 파일 없이 텍스트만 전송된 것 */
        @Schema(description = "Gemini에 실제 첨부된 파일 수", example = "2")
        private int attachedFileCount;

        /** 건너뛴 파일 목록 (미지원 형식, 파일 미존재 등) 프론트에서 사용자에게 안내하는 용도 */
    ...
    /** Gemini API Part: 텍스트 또는 파일(inlineData) 중 하나를 담습니다. null 필드는 JSON 직렬화에서 제외합니다. */
    @Getter
    @Builder
    @NoArgsConstructor
    @AllArgsConstructor
    ...
    public static class Part {
        ...
        /** 파일 파트 (Base64 인코딩된 파일 데이터) text와 inlineData 중 하나만 설정합니다. */
