 ...
 * Gemini API 연동 서비스
 *
 * <p>Google Gemini API를 호출하여 AI 응답을 생성합니다. {@code RestClient}를 사용한 동기 방식 HTTP 통신입니다.
 *
 * <p>첨부파일 메타데이터는 단건 조회로만 사용하고, 파일 시스템 I/O와 외부 Gemini API 호출은 트랜잭션으로 감싸지 않습니다. 긴 외부 호출이 DB 트랜잭션을
 * 점유하지 않도록 경계를 분리합니다.
 *
 * <p>API 키는 {@code application.properties}의 {@code gemini.api.key}로 관리합니다.
 */
@Slf4j
@Service
public class GeminiService {
     ...
     *
     * <p>Gemini API가 직접 처리 가능한 형식만 포함합니다. 미지원 형식(hwp, doc 등)은 전송 시 오류가 발생하므로 사전에 필터링합니다.
     */
     ...
     *
     * <ol>
     *   <li>프론트엔드 요청 DTO를 Gemini API 요청 형식으로 변환
     *   <li>Gemini generateContent API 호출
     *   <li>응답에서 텍스트 추출 후 프론트엔드 응답 DTO로 반환
     * </ol>
     *
    ...
    public GeminiDto.Response generate(GeminiDto.Request request) {
        ...
        List<String> requestedFlMpnIds =
                (request.getFlMpnIds() != null) ? request.getFlMpnIds() : Collections.emptyList();
        log.info(
                "Gemini 요청 - 모델: {}, 프롬프트 길이: {}, 요청 첨부파일 수: {}",
                model,
                request.getPrompt().length(),
                requestedFlMpnIds.size());
        ...
        log.info(
                "Gemini 파일 첨부 결과 - 성공: {}개, 건너뜀: {}개 {}",
                fileParts.size(),
                skippedFiles.size(),
                skippedFiles.isEmpty() ? "" : skippedFiles);
        ...
        try {
            GeminiDto.GeminiApiResponse apiResponse =
                    restClient
                            .post()
                            .uri(endpoint)
     ...
     *
     * <p>Parts 순서: [파일1, 파일2, ..., 텍스트 프롬프트] 파일을 먼저 보내야 Gemini가 컨텍스트를 올바르게 파악합니다.
     *
     * @param request 프론트엔드 요청 DTO
     * @param fileParts 이미 로딩된 파일 파트 목록
     * @return Gemini API 요청 바디
     */
    private GeminiDto.GeminiApiRequest buildApiRequest(
            GeminiDto.Request request, List<GeminiDto.Part> fileParts) {
        ...
        GeminiDto.Content userContent =
                GeminiDto.Content.builder().role("user").parts(parts).build();

        GeminiDto.GeminiApiRequest.GeminiApiRequestBuilder builder =
                GeminiDto.GeminiApiRequest.builder()
                        .contents(List.of(userContent))
                        .generationConfig(
                                GeminiDto.GenerationConfig.builder()
                                        .temperature(0.7)
        ...
        if (request.getSystemInstruction() != null && !request.getSystemInstruction().isBlank()) {
            builder.systemInstruction(
                    GeminiDto.SystemInstruction.builder()
                            .parts(
                                    List.of(
    ...
    private FilePartResult buildFilePartFromFlMngNo(String flMpnId) {
        ...
        if (mimeType == null) {
            return FilePartResult.skip(
                    "Gemini 미지원 형식: " + filename + " (지원: jpg/png/gif/webp/pdf/txt/csv)");
        }
        ...
        try {
            ...
            if (size > MAX_ATTACHMENT_BYTES) {
                log.warn(
                        "[Gemini] 첨부 크기 초과로 스킵 - flMpnId: {}, size: {}MB (제한: 20MB)",
                        flMpnId,
                        size / (1024 * 1024));
        ...
        log.info(
                "Gemini 파일 첨부 성공 - flMpnId: {}, 파일명: {}, MIME: {}, 크기: {}KB",
                flMpnId,
                filename,
                mimeType,
