...
public class FileTargetWriteAuthorizerRegistry {
     ...
     * 첨부 대상 쓰기 권한을 검증합니다.
     *
     * <p>먼저 <b>아는 종류인지</b>를 봅니다(SEC-14). 이 검사가 없으면 클라이언트가 임의의 새 종류 이름으로 첨부를 만들 수 있고, 그 종류에는
     * 아래 부모 권한 검사가 아예 붙지 않습니다. {@code pkColNm}이 {@code null}이면 이 검사를 건너뜁니다 — 메타 수정에서 종류를 바꾸지
     * 않는다는 뜻이기 때문입니다(업로드 경로는 {@code @RequestPart}가 필수로 강제합니다).
     *
