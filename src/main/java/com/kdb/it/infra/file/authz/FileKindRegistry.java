 ...
 * 통과시킵니다(레거시 보존). 그 상태에서는 클라이언트가 임의의 새 종류 이름으로 첨부를 만들 수 있고, 그 종류에는 부모 자원 쓰기 권한 검사가 붙지 않습니다. 알려진 종류만
 * 받도록 해서 그 경로를 닫습니다.
 *
 * <p><b>쓰기 판정기가 없는 종류도 허용합니다.</b> 등록된 쓰기 판정기는 넷뿐이라 그것만으로 좁히면 요구사항정의서·타당성검토표처럼 정상적으로 쓰이는 종류의 업로드가
 * 막힙니다. 이 목록의 목적은 <b>종류를 아는지</b>를 묻는 것이지 부모 권한 검사를 대신하는 것이 아닙니다.
 *
