 ...
 * 주관부서({@code SVN_DPM_C})와 현재 사용자를 비교한다. {@code Brdocm}은
 * 복합키(DOC_MNG_NO, DOC_VRS_SNO) 버전 엔티티이므로 최신 버전 로더로 조회한다.</p>
 */
@Component
...
public class RequirementDocFileReadAuthorizer implements FileReadAuthorizer {
     ...
     * 요구사항정의서 파일 읽기 가능 여부.
     *
     * @param file 대상 파일(부모 문서관리번호는 {@link Cfilem#getPkCone()})
     * @param user 현재 사용자(null이면 비인증 → 불가)
     * @return 관리자이거나, 최신 문서의 작성자(FST_ENR_USID) 또는 주관부서(SVN_DPM_C)가
     *         현재 사용자와 일치하면 true. 부모 ID가 없거나 문서를 찾지 못하면 false
     */
    @Override
