 ...
 *
 * <p>전문 조립({@link EaiMessageBuilder})과 전송을 연결한다. {@code eai.enabled=false}면 전문을 빌드·로깅만 하고 HTTP를
 * 호출하지 않는다(개발/CI 안전). 모든 실패는 예외를 전파하지 않고 {@link EaiResult}로 표현한다(부수효과 원칙). 민감정보(휴대폰/OTP) 노출 방지를 위해
 * 전문 전체를 평문 로깅하지 않고 마스킹한다.
 */
@Slf4j
@Service
public class EaiService {
    ...
    /** 로그용 마스킹 미리보기 — 헤더 앞 60바이트만 노출하고 개별부(민감정보 구간)는 가린다. 휴대폰번호/OTP가 포함된 개별부 전체 평문 로깅을 금지한다. */
    private String maskedPreview(byte[] message) {
        ...
        return prefix
                + (message.length > head ? "...(개별부 " + (message.length - head) + "바이트 마스킹)" : "");
    }
}
