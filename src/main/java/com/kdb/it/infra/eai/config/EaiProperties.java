 ...
 *
 * <p>시스템 식별자(IPP/PRM/PP)는 소스 하드코딩 없이 프로퍼티/프로파일로 관리한다. 비결정 값(시각/난수/IP·MAC)은 본 설정이 아니라 별도 시임 빈으로
 * 주입한다.
 *
...
public record EaiProperties(
    ...
    public EaiProperties {
        ...
        if (bzCS3.length() != 3)
            throw new IllegalStateException("eai.bz-c-s3 must be 3 chars: " + bzCS3);
        if (appC.length() != 3)
            throw new IllegalStateException("eai.app-c must be 3 chars: " + appC);
        if (appBzLv1C.length() != 2)
            throw new IllegalStateException("eai.app-bz-lv1-c must be 2 chars: " + appBzLv1C);
