-- ============================================================
-- 정보기술부문 계획 관련 DDL
-- ============================================================

-- 계획관리번호 채번 시퀀스 (PLN-{연도}-{4자리})
CREATE SEQUENCE S_PLN_MNG_NO
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE;

-- TPRMPP_BPLANM: 정보기술부문계획 마스터
CREATE TABLE TPRMPP_BPLANM (
    PLN_MNG_NO   VARCHAR2(32)   NOT NULL,   -- 계획관리번호 (PK)
    PLN_TP       VARCHAR2(16),              -- 계획구분 (신규, 조정)
    PLN_YY       VARCHAR2(4),              -- 대상년도 (YYYY)
    PLN_DTL_INF CLOB,                     -- 계획세부내용 (JSON 스냅샷)
    IT_PRJ_CONE  CLOB,                     -- IT프로젝트내용
    IT_BG_CONE   CLOB,                     -- IT예산내용
    IT_PRJ_RMK   CLOB,                     -- IT예산비고
    CPIT_BG_RMK  CLOB,                     -- 자본예산비고
    MNGC_BG_RMK  CLOB,                     -- 관리비예산비고
    TTL_BG       NUMBER(15, 2),            -- 총예산
    CPT_BG       NUMBER(15, 2),            -- 자본예산
    MNGC         NUMBER(15, 2),            -- 일반관리비
    DEL_YN       VARCHAR2(1)  DEFAULT 'N', -- 삭제여부
    FST_ENR_DTM  DATE,                     -- 최초생성시간
    FST_ENR_USID VARCHAR2(14),             -- 최초생성자
    GUID         VARCHAR2(38),             -- 일련번호
    GUID_PRG_SNO NUMBER(4, 0),             -- GUID진행일련번호
    LST_CHG_DTM  DATE,                     -- 마지막수정시간
    LST_CHG_USID VARCHAR2(14),             -- 마지막수정자
    CONSTRAINT PK_TPRMPP_BPLANM PRIMARY KEY (PLN_MNG_NO)
);

-- TPRMPP_BPLANA: 정보기술부문계획 관계 (BPROJM ↔ BPLANM N:N 매핑)
CREATE TABLE TPRMPP_BPLANA (
    ABUS_MNG_NO  VARCHAR2(32)   NOT NULL,  -- 프로젝트관리번호 (FK → TPRMPP_BPROJM)
    REQ_DOC_NO   VARCHAR2(32)   NOT NULL,  -- 요청문서번호 (FK → TPRMPP_BPLANM.PLN_MNG_NO)
    DEL_YN       VARCHAR2(1)  DEFAULT 'N', -- 삭제여부
    FST_ENR_DTM  DATE,                     -- 최초생성시간
    FST_ENR_USID VARCHAR2(14),             -- 최초생성자
    GUID         VARCHAR2(38),             -- 일련번호
    GUID_PRG_SNO NUMBER(4, 0),             -- GUID진행일련번호
    LST_CHG_DTM  DATE,                     -- 마지막수정시간
    LST_CHG_USID VARCHAR2(14),             -- 마지막수정자
    CONSTRAINT PK_TPRMPP_BPLANA PRIMARY KEY (ABUS_MNG_NO, REQ_DOC_NO)
);
