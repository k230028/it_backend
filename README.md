...
- **지정맥 클라이언트 증명 한계** — `mfa.md`의 지정맥 규격은 브라우저가 BioAgent에서 받는 결과 코드(`FE00`)만 정의하고, 서버가 독립 검증할 서명이나 기기 증명값을 정의하지 않습니다. 따라서 서버는 거래 nonce와 `FE00` 코드만 대조하며, 지정맥 결과는 FIDO·mOTP보다 위변조 저항성이 낮습니다. 향후 서버 검증 규격이 제공되면 `FingerVeinMfaProvider`만 교체합니다.
