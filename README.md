...
- **지정맥 해시 검증 미구현** — 현재 서버는 거래 nonce와 `FE00` 코드만 대조하므로 클라이언트 결과 코드에 의존합니다. `mfa.md`의 「지정맥인증 연계 보안방안」은 이를 보완할 해시 규격을 정의합니다: 업무 서버가 6자리 랜덤키를 발급하고, 지정맥인증 서버가 `년월일 + 사번 + 랜덤키 + 검증값(SUCC|FAIL) + 고정키`를 SHA-256으로 3회 해시해 돌려주면 업무 서버가 같은 값을 만들어 비교합니다. **아직 구현하지 않았습니다** — 해시 전달 경로, 고정키 주입, 사번 자릿수, 3회 해시의 정확한 연쇄 방식이 확정되어야 합니다(`../TASK.md` SEC-11). 도입 전까지 지정맥은 FIDO·mOTP보다 위변조 저항성이 낮습니다.
