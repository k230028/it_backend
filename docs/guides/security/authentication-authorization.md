...
접속 제한 대상 사용자는 자격증명이 맞아도 로그인할 수 없습니다. 부서코드(`BBR_C`)가 `9`로 시작하지 않으면서 행번(`ENO`)이 `O`/`o`로 시작하면 수동 로그인(비밀번호 검증 뒤)·MFA 완료·SSO 토큰 발급 모두에서 `AuthService.rejectIfLoginBlocked`가 거부하며, 로그인 실패 이력에 `접속 제한 대상 사용자`로 남깁니다. 이미 발급된 세션과 토큰 갱신은 대상이 아닙니다.
