...
- 토큰 수명은 `jwt.sso-verified-validity`(기본 60000ms)로 조정합니다. `sso-verified` 쿠키의 Max-Age도 `CookieUtil`이 같은 속성을 읽어 초 단위 올림(최소 1초)으로 맞추므로 토큰과 쿠키가 함께 만료됩니다. 리다이렉트 왕복만 버티면 되므로 늘릴 이유는 거의 없습니다.
